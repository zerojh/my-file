...
function get_name_by_cfgtype_id(cfg_type,index)
	local cfg = {SIPP="endpoint_sipphone",SIPT="endpoint_siptrunk",FXS="endpoint_fxso",FXO="endpoint_fxso",FXSO="endpoint_fxso",GSM="endpoint_mobile",CDMA="endpoint_mobile",RING="endpoint_ringgroup",ROUTE="endpoint_routegroup",IVR="ivr",VOLTE="endpoint_mobile"}
	if cfg[cfg_type] and index then
		if "FXS" == cfg_type or "FXO" == cfg_type and "GSM" == cfg_type and "CDMA" == cfg_type and "VOLTE" == cfg_type then
			return cfg_type
		else
	...
	for i=1,MAX_SIP_EXTENSION do
		for k,v in pairs(endpoint) do
			if v.index and i == tonumber(v.index) then
				...
				if not v.call_pickup or v.call_pickup == "ringgrp" then
					more_info[cnt] = i18n.translate("Call Pickup").." : "..i18n.translate("Ring Group").."<br>"
				elseif v.call_pickup == "extension" then
					more_info[cnt] = i18n.translate("Call Pickup").." : "..i18n.translate("Local Extension").."<br>"
				else
					more_info[cnt] = i18n.translate("Call Pickup").." : "..i18n.translate("Off").."<br>"
				end
				more_info[cnt] = more_info[cnt]..i18n.translate("Call Waiting").." : "..((v.waiting and "Activate" == v.waiting) and i18n.translate("On") or i18n.translate("Off")).."<br>"
		...
		for k,v in pairs(endpoint) do
			if v.index and v.name and i == tonumber(v.index) and "fxs" == v['.type']then
				for port=1,luci.version.license.fxs do
					...
					if v["did_"..port] and "table" == type(v["did_"..port]) then
						tmp[2] = table.concat(v["did_"..port],"|")
					else
					...
					if not v["call_pickup_"..port] or v["call_pickup_"..port] == "ringgrp" then
						more_info[cnt] = more_info[cnt]..i18n.translate("Call Pickup").." : "..i18n.translate("Ring Group").."<br>"
					elseif v["call_pickup_"..port] == "extension" then
						more_info[cnt] = more_info[cnt]..i18n.translate("Call Pickup").." : "..i18n.translate("Local Extension").."<br>"
					else
						more_info[cnt] = more_info[cnt]..i18n.translate("Call Pickup").." : "..i18n.translate("Off").."<br>"
					end
	...
	for i=1,MAX_RING_GRP do
		for k,v in pairs(ringgrp) do
			if v.index and v.name and i == tonumber(v.index) then
				...
				if v.ringtime then
					more_info[cnt] = more_info[cnt]..i18n.translate("Ring Time(5s~200s)").." : "..v.ringtime.."<br>"
				end
