								...
								sofia_dispatch_event_t *de, tagi_t tags[])
{
	...
	if (!gateway) {
		...
		if (!var_gateway && sip && sip->sip_to) {
			char tmp[64] = "";
			switch_snprintf(tmp, sizeof(tmp),"%s:%s",sip->sip_to->a_url->url_host,sip->sip_to->a_url->url_port);
			var_gateway = sofia_reg_find_gateway_by_realm(tmp);
		}

		if (!var_gateway && sip && sip->sip_to) {
			var_gateway = sofia_reg_find_gateway_by_addr(sip->sip_to->a_url->url_host);
		}
								...
								long exptime, sofia_regtype_t regtype, const char *to_user, switch_event_t **auth_params, long *reg_count, switch_xml_t *user_xml)
{
	...
	int first = 0;
	int passwd_len = 0;
	char base64_buff[128];
	const char *passwd = NULL;
	const char *passwd_base64 = NULL;
	...
	if (!(uparams = switch_xml_child(user, "params"))) {
		...
		for (param = switch_xml_child(uparams, "param"); param; param = param->next) {
			...
			if(!strcasecmp(var,"auth-profile") && 0 != strcasecmp(val,profile->name)){
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Register forbidden by param 'auth-profile',value=%s,profile-name=%s \n",val,profile->name);
				ret = AUTH_FORBIDDEN;
				goto end;
			}

			if (!strcasecmp(var, "sip-forbid-register") && switch_true(val)) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Register forbidden by param 'sip-forbid-register',value=%s \n",val);
				ret = AUTH_FORBIDDEN;
				goto end;
			}
			...
			if (!strcasecmp(var, "password-base64")) {
				passwd_base64 = val;
			}
	...
	if (!allow_empty_password && zstr(passwd) && zstr(a1_hash) && zstr(passwd_base64)) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "Empty password denied for user %s@%s\n", username, domain_name);
		ret = AUTH_FORBIDDEN;
		goto end;
	}

	if (zstr(passwd) && zstr(a1_hash) && zstr(passwd_base64)) {
		ret = AUTH_OK;
		goto skip_auth;
	}

	if (!zstr(passwd_base64))
	{
		passwd_len = base64_d(base64_buff,sizeof(base64_buff)-1,passwd_base64);
...
static int sofia_get_ip_by_hostname(char* hostname, char(*ip)[32], int max)
{
	struct hostent *he;
	...
	if (he)
	{
		...
		for (; *pptr!=NULL; pptr++)
	    {
			...
			if (i < max)
			{
				strcpy(ip[i++], switch_inet_ntop(AF_INET, &sa.sin_addr, buf, sizeof(buf)));
			}
			else
			{
...
static int sofia_check_gateway_by_hostname(char* realm, sofia_gateway_t *gateway, char* profile_name, char* username)
{
	/* 当gateway->register_realm是域名，而realm是ip地址时，需要解析下域名再比较 */
	if (realm && !gateway->deleted && gateway->register_realm && switch_check_ipaddr(realm) && !switch_check_ipaddr(gateway->register_realm)
		&& (!profile_name || (profile_name && gateway->profile && !strcasecmp(gateway->profile->name, profile_name)))
		&& (!username || (username && gateway->auth_username && !strcasecmp(gateway->auth_username, username))))
	{

		char realm_tmp[256] = {0};
		char* realm_ip = NULL;
		char* pos = NULL;
		char* realm_port = NULL;
		char ip[32][32];
		int i = 0;

		memset(ip, 0, sizeof(ip));

		if (0 < sofia_get_ip_by_hostname(gateway->register_realm, ip, 32))
		{
			strncpy(realm_tmp, realm, sizeof(realm_tmp));
			realm_ip = realm_tmp;
			if (NULL != (pos = strchr(realm_tmp, ':')))
			{
				*pos = '\0';
				realm_port = ++pos;
			}
...
sofia_gateway_t *sofia_reg_find_gateway_by_addr__(const char *file, const char *func, int line, const char *key)
{
	sofia_gateway_t *gateway = NULL;
	switch_hash_index_t *hi;
	const void *var;
	...
	for (hi = switch_core_hash_first(mod_sofia_globals.gateway_hash); hi; hi = switch_core_hash_next(&hi)) {
		...
		if (key && gateway && !gateway->deleted && gateway->addr && !strcasecmp(gateway->addr, key)) {
			break;
		}
		/* 当gateway->register_realm是域名，而realm是ip地址时，需要解析下域名再比较 */
		else if (sofia_check_gateway_by_hostname((char* )key, gateway, NULL, NULL))
		{
			break;
		}
		else {
...
sofia_gateway_t *sofia_reg_find_gateway_by_realm_profile__(const char *file, const char *func, int line, const char *realm,const char *profile_name)
{
	sofia_gateway_t *gateway = NULL;
	switch_hash_index_t *hi;
	const void *var;
	...
	for (hi = switch_core_hash_first(mod_sofia_globals.gateway_hash); hi; hi = switch_core_hash_next(&hi)) {
		switch_core_hash_this(hi, &var, NULL, &val);
		gateway = (sofia_gateway_t *) val;
		if (realm && gateway && !gateway->deleted && gateway->register_realm && !strcasecmp(gateway->register_realm, realm)
			&& gateway->profile && !strcasecmp(gateway->profile->name, profile_name)) {
			break;
		}
		/* 当gateway->register_realm是域名，而realm是ip地址时，需要解析下域名再比较 */
		else if (sofia_check_gateway_by_hostname((char*)realm, gateway, (char*)profile_name, NULL))
		{
			break;
		}
		else
...
sofia_gateway_t *sofia_reg_find_gateway_by_realm_profile_user__(const char *file, const char *func, int line, const char *realm,const char *profile_name,const char *username)
{
	sofia_gateway_t *gateway = NULL;
	switch_hash_index_t *hi;
	const void *var;
	...
	for (hi = switch_core_hash_first(mod_sofia_globals.gateway_hash); hi; hi = switch_core_hash_next(&hi)) {
		...
		gateway = (sofia_gateway_t *) val;
		if (realm && gateway && !gateway->deleted && gateway->register_realm && !strcasecmp(gateway->register_realm, realm)
			&& gateway->profile && !strcasecmp(gateway->profile->name, profile_name) && gateway->auth_username && !strcasecmp(gateway->auth_username, username)) {
			break;
		}
		/* 当gateway->register_realm是域名，而realm是ip地址时，需要解析下域名再比较 */
		else if (sofia_check_gateway_by_hostname((char*)realm, gateway, (char*)profile_name, (char*)username))
		{
			break;
		}
		else
...
int sofia_gateway_init_state_record(sofia_gateway_t *gateway)
{
	...
	if (!(gateway->state_record)) {
		if (!(record = switch_core_alloc(gateway->pool, sizeof(*record))))
			return 2;
		else {
			record->end_idx = -1;
			record->exist_num = 0;
			record->initialing = 1;
			gateway->state_record = record;
		}
	}
...
int sofia_gateway_traverse_state_record(sofia_gateway_t *gateway, switch_stream_handle_t *stream)
{
	...
	if (!(stream->write_function) || !(record = gateway->state_record))
		return 2;

	stream->write_function(stream, "\nState Record:\nDate\t\t\tState\n");
	if (record->initialing == 1 && record->end_idx != -1) {
		num = record->exist_num;
		start = (record->end_idx + 1) % num;
		end = record->end_idx;

		while (start != end) {
			stream->write_function(stream, "%s\t%s\n", switch_str_nil(record->time[start]), switch_str_nil(record->state[start]));
			start = (start + 1) % num;
		}
		stream->write_function(stream, "%s\t%s\n", switch_str_nil(record->time[start]), switch_str_nil(record->state[start]));
	}
