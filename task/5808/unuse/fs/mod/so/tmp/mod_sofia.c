...
switch_status_t sofia_on_destroy(switch_core_session_t *session)
{
	...
	if (tech_pvt) {
        ...
        if(tech_pvt->profile->session_count && switch_channel_direction(channel) == SWITCH_CALL_DIRECTION_INBOUND)
        {
            tech_pvt->profile->session_count--;
        }
		switch_mutex_unlock(tech_pvt->profile->flag_mutex);
...
/*
    //here is too late, will cause nth is not released,so move up
	if (tech_pvt->sofia_private) {
		*tech_pvt->sofia_private->uuid = '\0';
	}
*/
...
static switch_status_t sofia_receive_message(switch_core_session_t *session, switch_core_session_message_t *msg)
{
	...
	case SWITCH_MESSAGE_INDICATE_MEDIA_RENEG:
		{
			if (msg->string_arg) {
				...
				//sofia_clear_flag(tech_pvt, TFLAG_ENABLE_SOA);
				//实践证明,这里必须启用soa,不然200ok中的sdp不会在被sofia协议栈报上来,会导致协商编解码不正确
	...
	case SWITCH_MESSAGE_INDICATE_T38_DESCRIPTION:
		...
		break;
    case SWITCH_MESSAGE_INDICATE_T30_DESCRIPTION:
		{
			switch_t38_options_t *t30_options = switch_channel_get_private(tech_pvt->channel, "t30_options");
            ...
            switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG,
                                "SWITCH_MESSAGE_INDICATE_T30_DESCRIPTION\n");

            switch_core_media_set_codec(session, 1, 0);
	...
	case SWITCH_MESSAGE_INDICATE_UDPTL_MODE:
		...
		break;
    case SWITCH_MESSAGE_INDICATE_T30_MODE:
        {
            switch_t30_options_t *t30_options = switch_channel_get_private(channel, "t30_options");
	...
	if (argc > 0) {
		...
		if (!strcasecmp(argv[0], "gateway")) {
			if ((gp = sofia_reg_find_gateway(argv[1]))) {
				...
				stream->write_function(stream, "Address \t%s\n", switch_str_nil(gp->addr));
				stream->write_function(stream, "Port    \t%d\n", gp->sip_port);
				stream->write_function(stream, "Username\t%s\n", switch_str_nil(gp->register_username));
				stream->write_function(stream, "Auth Username\t%s\n", switch_str_nil(gp->auth_username));
				stream->write_function(stream, "Password\t%s\n", zstr(gp->register_password) ? "no" : "yes");
				stream->write_function(stream, "Reg From\t%s\n", switch_str_nil(gp->register_from));
				stream->write_function(stream, "Reg To  \t%s\n", switch_str_nil(gp->register_to));
				stream->write_function(stream, "Reg Contact\t%s\n", switch_str_nil(gp->register_contact));
				stream->write_function(stream, "Option From\t%s\n", switch_str_nil(gp->options_from_uri));
				stream->write_function(stream, "Option To\t%s\n", switch_str_nil(gp->options_to_uri));
				stream->write_function(stream, "Option User Agent\t%s\n", switch_str_nil(gp->options_user_agent));
				stream->write_function(stream, "Proxy    \t%s\n", switch_str_nil(gp->register_proxy));
				stream->write_function(stream, "Reg Proxy\t%s\n", switch_str_nil(gp->register_sticky_proxy));
				stream->write_function(stream, "Outbound Proxy\t%s\n", switch_str_nil(gp->outbound_sticky_proxy));
				stream->write_function(stream, "From Domain\t%s\n", switch_str_nil(gp->from_domain));
				stream->write_function(stream, "Extension\t%s\n", switch_str_nil(gp->extension));
				stream->write_function(stream, "Context \t%s\n", switch_str_nil(gp->register_context));
				stream->write_function(stream, "Expires \t%s\n", switch_str_nil(gp->expires_str));
				stream->write_function(stream, "Regry Sec\t%d\n", gp->retry_seconds);
				stream->write_function(stream, "Reg Fail Status\t%d\n", gp->failure_status);
				...
				sofia_gateway_traverse_state_record(gp, stream);
				stream->write_function(stream, "%s\n", line);
			...
			if ((argv[1]) && (profile = sofia_glue_find_profile(argv[1]))) {
				if (!argv[2] || (strcasecmp(argv[2], "reg") && strcasecmp(argv[2], "user"))) {
					...
					stream->write_function(stream, "Current Session  \t%u\n", profile->session_count);
					stream->write_function(stream, "DSCP-RTP(TOS)    \t%u\n", profile->rtp_dscp);
					stream->write_function(stream, "DSCP-SIP(TOS)    \t%u\n", profile->sip_dscp);
	...
	*new_session = NULL;
	...
	if (!strncasecmp(profile_name, "gateway/", 8)) {
		...
		*dest++ = '\0';
		...
		contact_user = switch_event_get_header(var_event, "sip_contact_user");

		if(contact_user)
		{
			{
				char *pre_contact_user = switch_mprintf("gw+%s", gateway_ptr->name);
				char *tmp_contact_str = switch_string_replace(gateway_ptr->register_contact,pre_contact_user,contact_user);
				if (params) {
					tech_pvt->invite_contact = switch_core_session_sprintf(nsession, "%s;%s", tmp_contact_str, params);
					tech_pvt->dest = switch_core_session_sprintf(nsession, "%s;%s", tech_pvt->dest, params);
				} else {
					tech_pvt->invite_contact = switch_core_session_strdup(nsession, tmp_contact_str);
				}
				free(pre_contact_user);
				free(tmp_contact_str);
			}
		}
		else
		{
	...
	sofia_glue_attach_private(nsession, profile, tech_pvt, outbound_profile->destination_number);
					...
					for (hi = switch_core_hash_first(mod_sofia_globals.profile_hash); hi; hi = switch_core_hash_next(&hi)) {
						...
						if ((profile = (sofia_profile_t *) val)) {
							if (!zstr(profile->extsipip) && !strcmp(profile->extsipip, old_ip4)) {
								...
								switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "SIP IP change detected, extsipip:%s\n", profile->extsipip);
							}

							if (!zstr(profile->extrtpip) && !strcmp(profile->extrtpip, old_ip4)) {
								profile->extrtpip = switch_core_strdup(profile->pool, new_ip4);
								switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "RTP IP change detected, extrtpip:%s\n", profile->extrtpip);
							}
						}
					}
				}
