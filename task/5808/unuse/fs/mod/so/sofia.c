...
static int g_byecnt_cur = -1;
static switch_time_t g_byetime = 0;

/* Á÷¿Ø ÓÉsip,freetdmµÈendpointÅÐ¶ÏºËÐÄ×ÊÔ´ÊÇ·ñÔÊÐíÉêÇë×ÊÔ´
type:
        0:½áÊøºô½Ð ftdmÖÐÎªÉêÇëÒ»¸öÐÂµÄsession  sipÖÐÎªÊÕµ½Ò»¸öbyeÏûÏ¢
        1 ÐÂµÄºô½Ð ÊÇ·ñÎªÐÂµÄºô½Ð ftdmÖÐÎªÉêÇëÒ»Â·ºô½Ð(fxs¿ÚÊÕºÅokºó·¢Æðºô½ÐÊ±)  sipÖÐÎªÊÕµ½Ò»¸öinviteÏûÏ¢
is_reject:ÊÇ·ñÖ±½Ó¾Ü¾ø  0 Ö±½Ó¾Ü¾ø ftdmÖÐºô½ÐÊÍ·ÅÊ±¿ÉÖ±½Ó¾Ü¾ø
                        1 ²ßÂÔ¾Ü¾ø sipÖÐÊÕµ½byeÏûÏ¢Ê±Ñ¡ÔñÐÔ(²ßÂÔÐÔ)µÄ¾Ü¾ø,²ßÂÔÓÉ¸Ãº¯Êý¾ö¶¨


µ÷ÓÃÊ± */
static uint32_t sofia_call_flow_control(uint32_t call_type, uint32_t is_reject, void * arg)
{
    sofia_profile_t * profile = (sofia_profile_t *)arg;
    int byecnt = 0;
    uint32_t core_session_count = switch_core_session_count();
    uint32_t core_session_limit = switch_core_session_limit(0);
    uint32_t session_count = profile->session_count;
    uint32_t session_limit = profile->max_proceeding?profile->max_proceeding:(core_session_limit/2);
    ...
    if (call_type == 0)
    {
        ...
        {
            /* ¼ÆËãÄÜ½ÓÊÜµÄbyeÏûÏ¢ */
            if (session_count >= session_limit * 70 / 100)
            {
                /* 70% ¿É½ÓÊÜ2¸öbyeÏûÏ¢ */
                byecnt = 2;
            }
            if (session_count >= session_limit * 30 / 100)
            {
                /* 30% ¿É½ÓÊÜ4¸öbyeÏûÏ¢ */
                byecnt = 4;
            }
    ...
    else if (call_type == 1)
    {
        ...
        /* µ±Ç°ºô½Ð´óÓÚlimit£¬Ö±½Ó¾Ü¾ø */
        if (session_count >= session_limit)
        {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "Over Profile Limit! %d\n", session_limit);
            return 2;
        }

        if (core_session_count >= core_session_limit)
        {
            switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "Over Core Session Limit! %d\n", core_session_limit);
            return 2;
        }

        if(profile->sps_limit)
        {
            /* Ã¿ÃëÖÓµÄºô½ÐÁ¿´óÓÚsps */
            if(profile->lastcheck <  (now - 1000000))
            {
                profile->sps = profile->sps_limit;
                profile->lastcheck = now;
            }
            /*ºô½ÐÆµÂÊ¹ý¿ì,Ôò¾Ü¾ø*/
            if((profile->sps--) <= 0)
            {
                switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "Over SPS Limit!\n");
                return 0;
            }
        }
								...
								sofia_dispatch_event_t *de, tagi_t tags[])
{
	...
	case nua_r_authenticate:
		...
		if (status >= 500) {
			if (sofia_private && !zstr(sofia_private->gateway_name)) {
				sofia_gateway_t *_gateway = NULL;

				if ((_gateway = sofia_reg_find_gateway(sofia_private->gateway_name))) {
					_gateway->state = REG_STATE_FAILED;
					_gateway->failure_status = status;
					sofia_reg_release_gateway(_gateway);
				}
			} else {
				nua_handle_destroy(nh);
			}
...
void sofia_queue_message(sofia_dispatch_event_t *de)
{
	...
	if (SWITCH_STATUS_SUCCESS != switch_queue_push(mod_sofia_globals.msg_queue, de))
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "push de to queue failed\n");
	}
}
						  ...
						  tagi_t tags[])
{
	...
	case nua_i_info:
		...
		/* 这里只限制invite  否则呼叫器极限情况下sess_count >= sess_max时，
		 * register包也会回复503 分机会掉注册,并且没有将nh释放掉 导致内存泄露
		 */
		if (!sofia_private && event == nua_i_invite) {
	...
	if (event == nua_i_invite && !sofia_private) {
		...
		if ((profile->max_proceeding && (profile->session_count+1) > profile->max_proceeding) || !sip || !sip->sip_call_id || zstr(sip->sip_call_id->i_id)) {
			...
			nua_handle_unref(nh);
			nua_stack_unref(nua);
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "FAIL:max_proceeding:%d, session_count:%d\n", profile->max_proceeding, profile->session_count);
		...
		if (session) {
			...
			if(sip->sip_contact && sip->sip_contact->m_url)
			{
				switch_channel_set_variable_printf(channel, "sip_contact_addr", "%s:%s", sip->sip_contact->m_url->url_host,sip->sip_contact->m_url->url_port);
			}

			if(sip->sip_from && sip->sip_from->a_url)
			{
				switch_channel_set_variable_printf(channel, "sip_from_addr", "%s:%s", sip->sip_from->a_url->url_host,sip->sip_from->a_url->url_port);
			}

			if(sip->sip_to->a_url->url_user)
			{
				switch_channel_set_variable_printf(channel,"sip_req_user","%s",sip->sip_to->a_url->url_user);
			}
			...
			if (sip->sip_request->rq_url->url_params) {
				...
				if(gw_param_name)
				{
					switch_channel_set_variable_printf(channel, "sip_gateway", "%s", gw_param_name);
				}
			}
...
void *SWITCH_THREAD_FUNC sofia_profile_thread_run(switch_thread_t *thread, void *obj)
{
	...
	do {
		profile->nua = nua_create(profile->s_root,	/* Event loop */
								  ...
								  TAG_IF(sofia_test_pflag(profile, PFLAG_NO_CONNECTION_REUSE),
										 TPTAG_REUSE(0)),
                                  NTATAG_FLOW_CTL(sofia_call_flow_control),
                                  NTATAG_FLOW_CTL_ARG(profile),
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Write lock before %s\n", profile->name);
	switch_thread_rwlock_wrlock(profile->rwlock);
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Write lock after %s\n", profile->name);

	//su_home_unref(profile->home);
	su_root_destroy(profile->s_root);
	//pool = profile->pool;

	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Del profile before %s\n", profile->name);
	sofia_glue_del_profile(profile);
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Del profile after %s\n", profile->name);
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Write unlock before %s\n", profile->name);
	switch_thread_rwlock_unlock(profile->rwlock);
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Write unlock after %s\n", profile->name);
	...
	for (gateway_tag = switch_xml_child(gateways_tag, "gateway"); gateway_tag; gateway_tag = gateway_tag->next) {
		...
		if ((gateway = switch_core_alloc(profile->pool, sizeof(*gateway)))) {
			...
			char *register_str = "true", *scheme = "Digest",
				...
				*auth_username = NULL,
				*password = NULL,
				*password_base64 = NULL,base64_buff[128],
			...
			for (; param; param = param->next) {
				...
				if (!strcmp(var, "register")) {
					...
					password = val;
				} else if (!strcmp(var, "password-base64")) {
					password_base64 = val;
				...
				} else if (!strcmp(var, "reg-url-with-transport")) {
					reg_url_with_transport = val;
				}
			}
...
*/
				...
				if(!zstr(password_base64))
				{
					passwd_len = base64_d(base64_buff,sizeof(base64_buff)-1,password_base64);
					if(passwd_len > sizeof(base64_buff))
					{
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "ERROR: Password too long!\n");
						goto skip;
					}
					else
					{

						base64_buff[passwd_len] = '\0';
						password = base64_buff;
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "decoce base64 password:%s -> %s,%s\n",password_base64,base64_buff,password);
					}
				}
			} else {
				if (zstr(username)) {
			...
			gateway->addr = switch_core_strdup(gateway->pool,realm);
			if ((p = strchr(gateway->addr, ':'))) {
				*p = '\0';
			}

			if ((p = strchr(realm, ':'))) {
				*p++;
				gateway->sip_port = atoi(p);
			}
			...
			gateway->contact_params = switch_core_strdup(gateway->pool, params);
			...
			if (!zstr(register_transport) && !switch_stristr("transport=", proxy)) {
				if(switch_true(reg_url_with_transport))
		...
		for (xprofile = switch_xml_child(profiles, "profile"); xprofile; xprofile = xprofile->next) {
			...
			if (!(settings = switch_xml_child(xprofile, "settings"))) {
				...
				if (!profile_already_started) {
					...
					/* Ä¬ÈÏ±»½ÐºÅÂë À´×ÔtoÓòÖÐ */
					sofia_clear_pflag(profile, PFLAG_DESTNUMBER_SIP_REQUEST_URL);
					sofia_set_pflag(profile, PFLAG_DESTNUMBER_SIP_TO);
					/* Ä¬ÈÏÖ÷½ÐºÅÂë À´×ÔfromÓòÖÐµÄuser */
					sofia_clear_pflag(profile, PFLAG_CALLERNUMBER_FROM_DISPLAY);
					sofia_set_pflag(profile, PFLAG_CALLERNUMBER_FROM_USER);
				...
				for (param = switch_xml_child(settings, "param"); param; param = param->next) {
					...
					if (!strcasecmp(var, "debug")) {
					...
					}else if (!strcasecmp(var, "sps-limit") && val) {
						int limit = atoi(val);

						if (limit >= 0) {
							profile->sps_limit = limit;
						}
					...
					} else if (!strcasecmp(var, "dest-number-src")) {
						if (!strcasecmp(val, "sip_req_url")) {
							sofia_set_pflag(profile, PFLAG_DESTNUMBER_SIP_REQUEST_URL);
							sofia_clear_pflag(profile, PFLAG_DESTNUMBER_SIP_TO);
						} else if (!strcasecmp(val, "sip_to")) {
							sofia_set_pflag(profile, PFLAG_DESTNUMBER_SIP_TO);
							sofia_clear_pflag(profile, PFLAG_DESTNUMBER_SIP_REQUEST_URL);
						}
					} else if (!strcasecmp(var, "caller-number-src")) {
						if (!strcasecmp(val, "sip_from_user")) {
							sofia_set_pflag(profile, PFLAG_CALLERNUMBER_FROM_USER);
							sofia_clear_pflag(profile, PFLAG_CALLERNUMBER_FROM_DISPLAY);
						} else if (!strcasecmp(val, "sip_from_display")) {
							sofia_set_pflag(profile, PFLAG_CALLERNUMBER_FROM_DISPLAY);
							sofia_clear_pflag(profile, PFLAG_CALLERNUMBER_FROM_USER);
						}
						...
						if (!zstr(val)) {
							...
							if (!strcmp(val, "0.0.0.0")) {
								...
								if (switch_find_interface_ip(buf, sizeof(buf), NULL, ifname, family) == SWITCH_STATUS_SUCCESS) {
									ip = buf;
									switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Using %s IP for interface %s for ext-rtp-ip\n", ip, val+10);
								} else {
									switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Unknown IP for interface %s for ext-rtp-ip\n", val+10);
								}
						...
						if (!zstr(val)) {
							...
							if (!strcasecmp(val, "0.0.0.0")) {
								...
								if (switch_find_interface_ip(buf, sizeof(buf), NULL, ifname, family) == SWITCH_STATUS_SUCCESS) {
									ip = buf;
									switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Using %s IP for interface %s for ext-sip-ip\n", ip, val+10);
								} else {
									switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Unknown IP for interface %s for ext-sip-ip\n", val+10);
								}
							}else if (!strcasecmp(val, "auto-nat")) {
								ip = NULL;
					...
					} else if (!strcasecmp(var, "rtp-autofix-payload")) { // add by lamont , 2014-11-6
						if (switch_true(val)) {
							sofia_set_media_flag(profile, SCMF_AUTOFIX_PAYLOAD);
						} else {
							sofia_clear_media_flag(profile, SCMF_AUTOFIX_PAYLOAD);
						}
					...
					} else if (!strcasecmp(var, "rtp-dscp")) {
						int rtp_dscp = atoi(val);
						profile->rtp_dscp = rtp_dscp;
					} else if (!strcasecmp(var, "sip-dscp")) {
						int sip_dscp = atoi(val);
						profile->sip_dscp = sip_dscp;
					} else if (!strcasecmp(var, "allow-unknown-call")) {
						if (switch_true(val)) {
							sofia_set_pflag(profile, PFLAG_ALLOW_UNKNOWN_CALL);
						} else {
							sofia_clear_pflag(profile, PFLAG_ALLOW_UNKNOWN_CALL);
						}
					}
				}
				...
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "profile->extsipip:%s\n", profile->extsipip?profile->extsipip:"null");
				if (switch_nat_get_type()) {
					...
					if (ip && !strchr(profile->sipip, ':')) {
						...
						if(strcasecmp(profile->sipip, profile->extsipip)){
							sofia_set_pflag(profile, PFLAG_AUTO_NAT);
							switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "NAT detected setting external ip to %s, profile->extsipip:%s\n", ip, profile->extsipip?profile->extsipip:"null");
						}
						else
						{
							sofia_clear_pflag(profile, PFLAG_AUTO_NAT);
							switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, "NAT off\n", ip);
						}
					}
					switch_safe_free(ip);
				}
									  ...
									  tagi_t tags[])
{
	...
	if (sip && session) {
		...
		else if (status > 299 && switch_channel_test_app_flag_key("T30", tech_pvt->channel, CF_APP_T30_REQ)) {
			switch_channel_set_private(channel, "t30_options", NULL);
			switch_channel_clear_app_flag_key("T30", tech_pvt->channel, CF_APP_T30);
			switch_channel_clear_app_flag_key("T30", tech_pvt->channel, CF_APP_T30_REQ);
			switch_channel_set_app_flag_key("T30", tech_pvt->channel, CF_APP_T30_FAIL);
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "%s T30 invite failed\n", switch_channel_get_name(tech_pvt->channel));
		}
				...
				if ((a_session = switch_core_session_locate(br)) && (a_channel = switch_core_session_get_channel(a_session))) {
					...
					if (sofia_test_pflag(profile, PFLAG_MANUAL_REDIRECT)) {
						if (!(v = switch_channel_get_variable(channel, "outbound_redirect_info"))) {
							...
							switch_channel_set_variable(a_channel, "redirect_transfer_val", "true");
							switch_channel_set_variable(a_channel, "destination_number", p_contact->m_url->url_user);
							switch_channel_set_variable(channel, "destination_number", p_contact->m_url->url_user);
			...
			tech_pvt->mparams.hold_laps = 0;
				...
				if (switch_channel_test_flag(channel, CF_PROXY_MODE) || switch_channel_test_flag(channel, CF_PROXY_MEDIA)) {
					...
					if (match) {
						...
						/* ·¢ËÍSWITCH_MESSAGE_RTPMEDIA_CHANGE_EVENT¸ø¶Ô¶Ë£¬¶Ô¶Ë¸üÐÂÃ½Ìå */
                        if (switch_core_session_get_partner(session, &other_session) == SWITCH_STATUS_SUCCESS)
                        {
    						switch_core_session_message_t *msg;
                            msg = switch_core_session_alloc(other_session, sizeof(*msg));
    						msg->message_id = SWITCH_MESSAGE_RTPMEDIA_CHANGE_EVENT;
    						msg->from = __FILE__;
    						msg->string_arg = switch_core_session_strdup(other_session, r_sdp);
                			switch_core_session_queue_message(other_session, msg);
					        switch_core_session_rwunlock(other_session);
								...
								sofia_dispatch_event_t *de, tagi_t tags[])
{
					...
					if ((b_private = nua_handle_magic(bnh))) {
						...
						if (deny_refer_requests) {
							...
							if ((tmp = switch_core_session_locate(br_a))) {
								...
								switch_channel_set_variable(tchannel, "att_transfer_val", "true");
								switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_NOTICE, "a name:%s\n",
								switch_channel_get_name(tchannel));
								switch_core_session_rwunlock(tmp);
							}
							else
							{
								switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING, "switch_core_session_locate(br_a) error\n");
							}
							...
							if ((tmp = switch_core_session_locate(br_b))) {
								...
								switch_channel_set_variable(tchannel, "att_transfer_val", "true");
								switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_NOTICE, "b name:%s\n",
								switch_channel_get_name(tchannel));
							...
							{
								switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_WARNING, "switch_core_session_locate(br_b) error\n");
							}
	...
	if (exten) {
		...
		if (!zstr(br) && (b_session = switch_core_session_locate(br))) {
			...
			switch_channel_set_variable(channel, "destination_number", exten);
			switch_channel_set_variable(b_channel, "destination_number", exten);
			switch_channel_set_variable(channel, "blind_transfer_val", "true");
			switch_channel_set_variable(b_channel, "blind_transfer_val", "true");
...
void sofia_handle_sip_i_invite(switch_core_session_t *session, nua_t *nua, sofia_profile_t *profile, nua_handle_t *nh, sofia_private_t *sofia_private, sip_t const *sip, sofia_dispatch_event_t *de, tagi_t tags[])
{
	...
	profile->ib_calls++;
	tech_pvt = switch_core_session_get_private(session);
	chan_name = switch_channel_get_name(tech_pvt->channel);

	if(!strncasecmp(chan_name, "sofia/unknown/",14) && !sofia_test_pflag(profile, PFLAG_ALLOW_UNKNOWN_CALL))
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "(%s) Rejected by 'reject unknown call!'\n",chan_name);
		nua_respond(nh, SIP_403_FORBIDDEN, TAG_END());
		goto fail;
	}

	if(!strncasecmp(chan_name, "sofia/gateway/",14))
	{
		gw_name = switch_channel_get_variable(tech_pvt->channel, "sip_gateway");
		gateway = sofia_reg_find_gateway(gw_name);

	    if (gateway) {
	        if(strcasecmp(profile->name,gateway->profile->name))
	        {
	        	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_WARNING, "(%s) Rejected because gateway[%s] isn't belong to profile[%s]\n",chan_name,gw_name,profile->name);
				nua_respond(nh, SIP_403_FORBIDDEN, TAG_END());
				sofia_reg_release_gateway(gateway);
				goto fail;
	        }
	...
	/* 这里已经创建了session  再判断是否无意义 并且会导致内存泄露 */
	if (!session || !sofia_test_pflag(profile, PFLAG_RUNNING))
	{
	...
	if (sip->sip_request->rq_url) {
		...
		if (sofia_test_pflag(profile, PFLAG_DESTNUMBER_SIP_REQUEST_URL))
		{
			if (sofia_test_pflag(profile, PFLAG_FULL_ID)) {
	...
	if (sofia_test_pflag(profile, PFLAG_DESTNUMBER_SIP_TO))
	{
		if (sip->sip_to && sip->sip_to->a_url) {
	...
	//	gw_param_name = switch_find_parameter(sip->sip_request->rq_url->url_params, "gw", switch_core_session_get_pool(session));
	//}
	//sofia_event_callbackÖÐÒÑ¾­²éÕÒ¹ý²¢±£´æ, by lamont
...
*/
	...
	/* Ö÷½ÐºÅÂëÀ´Ô´ */
	if (sofia_test_pflag(profile, PFLAG_CALLERNUMBER_FROM_DISPLAY))
	{
		profile_dup_clean(displayname, tech_pvt->caller_profile->caller_id_number, tech_pvt->caller_profile->pool);
		profile_dup_clean(displayname, tech_pvt->caller_profile->orig_caller_id_number, tech_pvt->caller_profile->pool);
	}
	else //if (sofia_test_pflag(profile, PFLAG_CALLERNUMBER_FROM_USER))
	{
  ...
  fail:
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_CRIT, "%s sofia_handle_sip_i_invite failed\n", chan_name);
	if(session)
		switch_channel_hangup(switch_core_session_get_channel(session),SWITCH_CAUSE_SWITCH_CONGESTION);
	return;
