...
[ "$SECTION_ID" = "myddns_ipv4" ] && [ -z "$domain" -o -z "$username" -o -z "$password" ] && critical_error "Service Configuration not correctly configured"
...
[ $SECTION_ID == "myddns_ipv4" ] && {
	...
	echo "Date:`date '+%Y-%m-%d %H:%M:%S'`, Service:DDNS, State:10 seconds for interfaces to fully come up" >> /ramlog/service_state_log
...
while : ; do
	...
	[ $VERBOSE_MODE -le 2 ] && {
		...
		[ $SECTION_ID == "myddns_ipv4" ] && {
			lock /tmp/service_state_log_lock
			echo "Date:`date '+%Y-%m-%d %H:%M:%S'`, Service:DDNS, State:$CHECK_SECONDS seconds (Check Interval) before continue" >> /ramlog/service_state_log
			lock -u /tmp/service_state_log_lock
		}
		sleep $CHECK_SECONDS
