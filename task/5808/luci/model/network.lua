...
function get_netinfo()
	...
	if luci.version.license and luci.version.license.lte then
		...
		lte_info.mode = i18n.translate(lte_info.mode).." / "..i18n.translate((tmp:match("lte_mode%s*=%s*([a-zA-Z0-9]+)\n") or "UNKNOWN"))
...
function profile_network_init()
	...
	if wan_section then
		...
		if drv_str == "rt2860v2_ap" and drv_str == "rt2860v2_sta" then
