...
function Cursor.check_cfg_deps(self,config,section,param)
	if "" ~= param then
		...
		local tmp_tb = util.split(param," ") or {}
		...
		for _,node in pairs(tmp_tb) do
			...
			elseif "endpoint_extension.filter" == node then
				for k,v in pairs(self:get_all("endpoint_sipphone") or {}) do
					if (v.callin_filter_blacklist and v.callin_filter_blacklist == cur_index) or (v.callin_filter_whitelist and v.callin_filter_whitelist == cur_index) or (v.callout_filter_blacklist and v.callout_filter_blacklist == cur_index) or (v.callout_filter_whitelist and v.callout_filter_whitelist == cur_index) then
				...
				for k,v in pairs(self:get_all("endpoint_fxso") or {}) do
					if v[".type"] == "fxs" then
						if (v.callin_filter_blacklist_1 and v.callin_filter_blacklist_1 == cur_index) or (v.callin_filter_whitelist_1 and v.callin_filter_whitelist_1 == cur_index) or (v.callout_filter_blacklist_1 and v.callout_filter_blacklist_1 == cur_index) or (v.callout_filter_whitelist_1 and v.callout_filter_whitelist_1 == cur_index) then
							return "alert('"..i18n.translatef("Can not delete, it is being used in <%s> !",tostring(i18n.translate("FXS Extension"))).."');return false",false
						elseif (v.callin_filter_blacklist_2 and v.callin_filter_blacklist_2 == cur_index) or (v.callin_filter_whitelist_2 and v.callin_filter_whitelist_2 == cur_index) or (v.callout_filter_blacklist_2 and v.callout_filter_blacklist_2 == cur_index) or (v.callout_filter_whitelist_2 and v.callout_filter_whitelist_2 == cur_index) then
							return "alert('"..i18n.translatef("Can not delete, it is being used in <%s> !",tostring(i18n.translate("FXS Extension"))).."');return false",false
						end
					end
				end
