#!/bin/sh
	...
	build_oem_info()
	{
		...
		if [ "yqsd" == ${brand} ]; then
			cp oem_database/yqsd/oem $tmpconf
			cp oem_database/yqsd/postinst $tmpctrl
			cp oem_database/yqsd/postrm $tmpctrl
			cp oem_database/yqsd/luci $tmpconf -rf
			cp oem_database/yqsd/usr $tmpconf -rf
		else
	...
	build_cloud()
	{
		case $1 in
			opkg)
				...
				local tmpctrl=$tmpname/CONTROL
				local tmpcloud=$tmpname$firmware_upgrading_temp_dir/cloud/
				...
				cp $sourcedir/cloud/etc $tmpcloud -r
	...
	build_freeswitch()
	{
		case $1 in
			...
			opkg)
				...
				local tmpctrl=$tmpname/CONTROL
				local tmpinitd=$tmpname/etc/init.d
				local tmpbin=$tmpname/bin
				local tmpusrbin=$tmpname/usr/bin
				local tmplib=$tmpname/lib
				local tmpfsmod=$tmpname/lib/fsmod
				local tmpetc=$tmpname/etc/freeswitch
				...
				#准备安装脚本
				build_opkg_script $tmpctrl 	$tmpinitd freeswitch 99 16 '-base \/etc\/freeswitch -conf \/etc\/freeswitch\/conf -db \/tmp\/fsdb -log \/var\/log -mod \/lib\/fsmod -nf -np -nc -nocal -product UBG1000'
