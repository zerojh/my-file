	...
	for k,v in pairs(uci:get_all("endpoint_fxso") or {}) do
		if "fxs" == v['.type'] and v.index and "Enabled" == v.status then
			if luci.version.license.fxs > 1 then
				table.insert(forward_extension_dst,{index="FXS/"..v.index.."/1",name=translate("FXS Extension").." / "..v.number_1})
				table.insert(forward_extension_dst,{index="FXS/"..v.index.."/2",name=translate("FXS Extension").." / "..v.number_2})
			else
				table.insert(forward_extension_dst,{index="FXS/"..v.index.."/1",name=translate("FXS Extension")})
			end
		end
	end
end
...
for i=1,32 do
	for k,v in pairs(uci:get_all("endpoint_sipphone") or {}) do
		if k ~= current_section and v.index and v.name and tonumber(v.index) == i and "Enabled" == v.status and v.user then
			table.insert(forward_extension_dst,{index="SIPP-"..v.index,name=translate("SIP Extension").." / "..v.name.." / "..v.user})
			break
		end
	end
end
