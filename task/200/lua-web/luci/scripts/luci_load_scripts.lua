...
function compare_xml_cfg(cfg,xml)
	...
	local cfg_tb = uci:get_all(cfg) or {}
	local root
	local flag = true
	local url
	local node
...
function get_all_configed_gw()
	...
	trunk = uci:get_all("endpoint_siptrunk") or {}
	...
	trunk = uci:get_all("endpoint_fxso") or {}
	for k,v in pairs(trunk) do
	...
	trunk = uci:get_all("endpoint_mobile") or {}
	for k,v in pairs(trunk) do
		if "Enabled" == v.status then
	...
	if network_model then
		if network_model == "route" then
			...
			if uci:get_all("network","wan") then
				...
				uci:set("network","wan","hostname",uci:get("system","main","hostname") or "UC200")
		...
		elseif network_model == "bridge" then
			...
			uci:set("network","lan","hostname",uci:get("system","main","hostname") or "UC200")
			uci:set("wireless","wifi0","network","lan")
			uci:set("wireless","wifi0","mode","ap")
			uci:set("wireless","wifi0","network","lan")
			if uci:get_all("network","wan") then
...
function refresh_wireless_config()
	local wireless_tb = uci:get_all("wireless") or {}
	...
	wireless_tb = uci:get_all("wireless") or {}
...
--把 luci.sys.user.setpasswd 这个函数抽出来单独放这里调用，否则升级时直接调用luci.sys可能会出错
function user_setpasswd(username, password)
	if password then
...
function user_getpasswd(username)
...
function user_checkpasswd(username, pass)
	local pwh, pwe = user_getpasswd(username)
	if pwe then
...
function sync_user_to_system(is_restore)
	local user_list = uci:get_all("user") or {}
	local current_sys_user_list = {}
	local passwd_file = util.exec("cat /etc/passwd | cut -d ':' -f 1") or ""
	local passwd_file_user_tb = util.split(passwd_file,"\n")
	for k,v in ipairs(passwd_file_user_tb) do
		if "root" ~= v and "daemon" ~= v and "ftp" ~= v and "network" ~= v and "nobody" ~= v and "admin" ~= v and "api" ~= v and "" ~= v then
			current_sys_user_list[v]={}
		end
	end

	local web_access_user="\"admin\""

	for k,v in pairs(user_list) do
		if "user" == v[".type"] and v.name and "root" ~= v.name and "daemon" ~= v.name and "ftp" ~= v.name and "network" ~= v.name and "nobody" ~= v.name and "admin" ~= v.name and "api" ~= v.name then
			if not current_sys_user_list[v.name] then
				current_sys_user_list[v.name]={newuser=true,group=v.group,password=v.password,status=v.status}
			else
				current_sys_user_list[v.name]={edituser=true,group=v.group,password=v.password,status=v.status}
			end
			web_access_user=web_access_user..",\""..k.."\""
			local year,month,day = v.expires:match("(%d+)%-(%d+)%-(%d+)")
			if year and month and day then
				local expires_epoch=os.time({year=year,month=month,day=day,hour=23,min=59,sec=59})
				uci:set("user",v.name,"expires_epoch",expires_epoch)
				uci:save("user")
				uci:commit("user")
			end
		end
	...
	for k,v in pairs(current_sys_user_list) do
		if v.newuser and "Enabled" == v.status then
			os.execute("(echo '"..v.password.."'; sleep 1; echo '"..v.password.."') | ".."adduser '"..k.."' -G "..v.group.." >/dev/null 2>&1")
		elseif v.edituser and "Enabled" == v.status then
			if not user_checkpasswd(k,v.password) then
				user_setpasswd(k,v.password)
				os.execute("grep \""..k.."\" /tmp/luci-sessions/* -l | xargs rm -rf")
				os.execute("adduser "..k.." "..v.group) --not handle group now
			elseif is_restore and fs.access("/etc/config/user_backup") then
				local k_oldpw = uci:get("user_backup",k,"password")
				if k_oldpw and v.password ~= k_oldpw then
					os.execute("grep \""..k.."\" /tmp/luci-sessions/* -l | xargs rm -rf")
				end
			end
		else
			os.execute("deluser "..k) --delete unuser user
			os.execute("grep \""..k.."\" /tmp/luci-sessions/* -l | xargs rm -rf")
		end
	end

	if is_restore and fs.access("/etc/config/user_backup") then
		for k,v in pairs(uci:get_all("user_backup") or {}) do
			if "user" == v[".type"] and v.name then
			 	if not uci:get_all("user",v.name) then
			 		os.execute("grep \""..v.name.."\" /tmp/luci-sessions/* -l | xargs rm -rf &")
			 	end
			end
		end
		os.execute("rm /etc/config/user_backup")
	end

	os.execute("sed -i 's/page.sysauth = .*/page.sysauth = {"..web_access_user.."}/g' /usr/lib/lua/luci/controller/admin/index.lua")
	os.execute("sed -i 's/.sysauth = .*/.sysauth = {"..web_access_user.."}/g' /usr/lib/lua/luci/controller/admin/servicectl.lua")
end
...
function load_scripts(param)
	...
	if param.mobile or param.restore or param.upgrade then
		applyed_list = add_to_list(applyed_list,"GSM/CDMA/VOLTE")
		fs.writefile("/tmp/fs-apply-status","Applying="..applyed_list)
		...
		if ret ~= 0 or not state then
			flag = true
			err = add_to_list(err, "GSM/CDMA/VOLTE")
		end
	end
	...
	if param.openvpn then
		...
		if fs.access("/etc/openvpn/my-vpn.conf") then
	...
	if param.ssh then
		exe("killall dropbear || /etc/init.d/dropbear stop")
		exe("sleep 1")
	...
	if param.user or param.restore or param.upgrade then
		sync_user_to_system(param.restore)
	end
