...
function reset_system_config()
	...
	uci:set("system","main","hostname",uci:get("oem",brand,"hostname") or "UC200")
	...
	os.execute("rm /etc/config/user /etc/log/clilog /etc/log/weblog /etc/log/pptpc_log /etc/log/l2tpc_log /etc/log/openvpnc_log /etc/log/uci_changes_log*")
end
...
function reset_network()
	...
	local uci = require "luci.model.uci".cursor()
	local brand = uci:get("oem","general","brand") or "unknown"
	local hostname = uci:get("oem",brand,"hostname") or "UC200"
	...
	--@ rm upnpc client config
	os.execute("rm /etc/config/upnpc")
