...
function sip()
	...
	local colgroup = {"5%","10%","6%","12%","10%","12%","10%","14%","14%","7%"}
	local content = {}
	local edit = {}
	local delchk = {}
	local uci_cfg = {}
	...
	for i=1,MAX_SIP_PROFILE do
		for k,v in pairs(profile) do
			if v.index and i == tonumber(v.index) then
				...
				more_info[cnt] = more_info[cnt]..i18n.translate("Progress Timeout(s)")..":"..(v.progress_timeout or 55).."<br>"
...
function number()
	...
	for i=1,MAX_NUMBER_PROFILE do
		for k,v in pairs(number) do
			if v.index and v.name and i == tonumber(v.index) then
				...
				delchk[cnt] = uci:check_cfg_deps("profile_number",k,"route.numberProfile endpoint_extension.filter")
	...
	if MAX_NUMBER_PROFILE == cnt then
		addnewable = false
	end
	luci.template.render("cbi/configlist",{
...
function number_learning()
	...
	local ds = require "luci.dispatcher"
	local i18n = require "luci.i18n"
	local MAX_NUMBERLEARNING_PROFILE = tonumber(uci:get("profile_param","global","max_numberlearning_profile") or "32")
	...
	if MAX_NUMBERLEARNING_PROFILE == cnt then
		addnewable = false
	end
	luci.template.render("cbi/configlist",{
