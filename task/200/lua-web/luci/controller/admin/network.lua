...
function index()
	...
	elseif luci.version.license and luci.version.license.volte then
		entry({"admin","network","lte"},cbi("admin_network/wan_lte_edit"),_("VoLTE"),30)
		entry({"admin","network","uplink"},cbi("admin_network/uplink_edit"),_("Uplink Config"),40)
	end
...
function port_map()
	...
	if MAX_FIREWALL_REDIRECT == cnt then
		addnewable = false
	end
...
function static_route_list()
	...
	if luci.version.license and luci.version.license.volte then
		interface_name={["wan"]="WAN",["lan"]="LAN",["wan2"]="VoLTE",["openvpn"]="OpenVPN",["ppp1701"]="L2TP",["ppp1723"]="PPTP"}
	else
		interface_name={["wan"]="WAN",["lan"]="LAN",["wan2"]="LTE",["openvpn"]="OpenVPN",["ppp1701"]="L2TP",["ppp1723"]="PPTP"}
	end
