...
function action_sms_route()
	...
	local MAX_SMSROUTE_PROFILE = tonumber(uci:get("profile_param","global","max_smsroute_profile") or "32")
	...
	for i=1,MAX_SMSROUTE_PROFILE do
	...
	if MAX_SMSROUTE_PROFILE == cnt then
		addnewable = false
	end
...
function get_name_by_cfgtype_id(cfg_type,index)
	local cfg = {SIPP="endpoint_sipphone",SIPT="endpoint_siptrunk",FXS="endpoint_fxso",FXO="endpoint_fxso",FXSO="endpoint_fxso",GSM="endpoint_mobile",SMS="endpoint_mobile",USSD="endpoint_mobile",CDMA="endpoint_mobile",RING="endpoint_ringgroup",ROUTE="endpoint_routegroup",IVR="ivr",VOLTE="endpoint_mobile"}
	if cfg[cfg_type] and index then
		if "FXS" == cfg_type or "FXO" == cfg_type or "GSM" == cfg_type or "CDMA" == cfg_type or "SMS" == cfg_type or "USSD" == cfg_type or "VOLTE" == cfg_type then
			return cfg_type
		else
...
function routegroup()
	...
	local ds = require "luci.dispatcher"
	local i18n = require "luci.i18n"
	local MAX_ROUTEGRP = tonumber(uci:get("profile_param","global","max_routegroup") or '32')
		...
		for k,v in pairs(routegrp) do
			if v.index and v.name and i == tonumber(v.index) then
				...
				for _,v in ipairs(v.members_select) do
					...
					if v:match("^SIPP") or v:match("^SIPT") or v:match("^GSM") or v:match("^CDMA") or v:match("^VOLTE") then
					...
					elseif vtype == "VOLTE" then
						v = tostring(i18n.translate("VoLTE Trunk"))
					end
					table.insert(td[3],v)
				end
...
function route()
	...
	local i18n = require "luci.i18n"
	local interface = uci:get("system","main","interface") or ""
	local MAX_ROUTE = tonumber(uci:get("profile_param","global","max_route") or '32')
...
function featurecode()
	...
	local i18n = require "luci.i18n"
	local enable_flg
	local MAX_FEATURE_CODE = tonumber(uci:get("profile_param","global","max_feature_code") or "32")
...
function action_ussd_get()
	...
	if luci.version.license then
		if luci.version.license.gsm then
			mod_type = "GSM"
		elseif luci.version.license.lte then
			mod_type = "LTE"
	...
	luci.template.render("admin_callcontrol/ussd",{refresh=refresh,info_send_msg=info_send_msg,enable_quit=enable_quit,mod_usable=mod_usable,mod_type=mod_type,num=num})
end
