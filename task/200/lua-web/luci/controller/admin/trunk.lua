...
function index()
	...
	if luci.version.license and luci.version.license.gsm then
		entry({"admin","trunk","mobile"},call("mobile_gsm"), "GSM",3)
	...
	elseif luci.version.license and luci.version.license.volte then
		entry({"admin","trunk","mobile"},call("mobile_volte"), "VoLTE",3)
		entry({"admin","trunk","mobile","mobile"},cbi("admin_trunk/mobile_edit"),nil,3).leaf = true
	end
...
function fxo()
	...
	local ds = require "luci.dispatcher"
	local i18n = require "luci.i18n"
	local MAX_FXSO_ENDPOINT = tonumber(uci:get("profile_param","global","max_fxso_endpoint") or "4")
			...
			if v.index and v.name and "fxo" == v['.type'] and i == tonumber(v.index) then
				...
				for port=1, total_port do
					...
					if "on" == v["port_"..port.."_reg"] and (v["port_"..port.."_server_1"] or v["port_"..port.."_server_2"] or v["authuser_"..port]) then
						...
						more_info[cnt] = i18n.translate("Display Name / Username Format")..": "..i18n.translate(from_field_v[v["sip_from_field_"..port]] or from_field_v["0"]).."<br>"
...
function mobile_gsm()
	...
	local th = {"Extension","Autodial Number","Register to SIP Server","Voice Codec","Band Type","Carrier","Input Gain","Output Gain","Status"}
	...
	local MAX_MOBILE_TRUNK = tonumber(uci:get("profile_param","global","max_mobile_trunk") or "2")
...
function mobile_volte()
	...
	local speedtype = {"Auto","FR","HR","EFR","AMR_FR","AMR_HR","FR & EFR","EFR & FR","EFR & HR","EFR & ARM_FR","AMR_FR & FR","AMR_FR & HR","AMR_FR & EFR","AMR_HR & FR","AMR_HR & HR","AMR_HR & EFR"}
	local bandtype = {"All","VoLTE 900","VoLTE 1800","VoLTE 1900","VoLTE 900 & VoLTE 1800","VoLTE 850 & VoLTE 1900"}
	...
	luci.template.render("cbi/configlist",{
		title = i18n.translate("Trunk / VoLTE"),
		colgroup = colgroup,
		th = th,
		content = content,
		more_info = more_info,
