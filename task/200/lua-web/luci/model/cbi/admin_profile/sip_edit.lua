...
local profile_sip = uci:get_all("profile_sip") or {}
...
if arg[2] == "edit" then
	...
	for i=1,MAX_SIP_PROFILE do
		local flag = true
		for k,v in pairs(profile_sip) do
...
elseif luci.version.license and luci.version.license.volte then
	localinterface:value("VoLTE","VoLTE")
end
...
progresstimeout = s:option(Value,"progress_timeout",translate("Progress Timeout(s)"))
progresstimeout.default = "55"
progresstimeout.rmempty = false
progresstimeout.datatype = "range(1,200)"
