#!/bin/sh
	...
	build_oem_info()
	{
		...
		if [ "bluewave/WP1" == ${brand} ]; then
			cp oem_database/bluewave/WP1/bin/* $tmpconf
			...
			cp oem_database/bluewave/WP1/luci $tmpconf -rf
			cp oem_database/bluewave/WP1/lib $tmpconf -rf
			cp oem_database/bluewave/WP1/usr $tmpconf -rf
		elif [ "bluewave/S5V" == ${brand} ]; then
			cp oem_database/bluewave/S5V/bin $tmpconf -r
			cp oem_database/bluewave/S5V/etc $tmpconf -r
			cp oem_database/bluewave/S5V/luci $tmpconf -r
			cp oem_database/bluewave/S5V/usr $tmpconf -r
			cp oem_database/bluewave/S5V/oem $tmpconf
			cp oem_database/bluewave/S5V/pldt_logo.png $tmpconf
		...
		elif [ "tirasoft" == ${brand} ]; then
			cp oem_database/tirasoft/tirasoft_logo.png $tmpconf -rf
			cp oem_database/tirasoft/oem $tmpconf -rf
			cp oem_database/tirasoft/tirasoft.lua $tmpconf -rf
			cp oem_database/tirasoft/pbx.htm $tmpconf -rf
			cp oem_database/tirasoft/postinst $tmpctrl
		else
	...
	build_filesystem()
	{
		case $1 in
			opkg)
				...
				local tmpctrl=$tmpname/CONTROL
				local tmpconf=$tmpname$firmware_upgrading_temp_dir/filesystem/
				...
				#准备安装脚本
				cp $filesystemdir/broadcastwatch $tmpconf
				cp $filesystemdir/libcurl.so.4.3.0 $tmpconf
				cp $filesystemdir/modify_xml.lua $tmpconf
				cp $filesystemdir/network_watch $tmpconf
   ...
   case ${cpu} in
	...
	s805-arm)
		...
		export STAGING_USR=/opt/toolchains/gcc-linaro-arm-linux-gnueabihf-4.9-2014.09_linux/arm-linux-gnueabihf/
			...
			echo "PKG_CONFIG_PATH=$PKG_CONFIG_PATH, libdir=$libdir"
			export ARCH=armv7
			export VAPI_FLAGS=-DPRODUCT_UC200
			export pbc_dest_dir=$libdir
			

			#公共的configure参数
            common_conf="--target=arm-linux-gnueabihf --host=arm-linux-gnueabihf --build=i686-pc-linux-gnu --enable-ipv6"

			#公共的cflags参数
			common_cflags="${gdb} -fasynchronous-unwind-tables -rdynamic -I$syslibdir/include/dpr -I$libdir/include -I$libdir/include/apr-1 -I$libdir/include/curl -I$libdir/include/dpr -DPRODUCT_UC200"
			common_cppflags="-DPRODUCT_UC200"

			#公共的ldflags参数
			common_ldflags="-L$syslibdir/lib -L$libdir/lib -lstdc++"
        ...
        mipsel)
			...
			#公共的ldflags参数
			common_ldflags="-L$syslibdir/lib -L$libdir/lib -L$STAGING_USR/lib"
