...
function index()
	...
	entry({"admin","system","security","user"},call("user_add_edit"),nil,2).leaf = true
	...
	entry({"admin","system","ctl_xml"},call("ctl_xml"),_("XML Debug"),15)
...
function get_diagnostics(m)
	...
	if m:match("gsm") then
		...
		if ip and dtmf_recv then
			s = s..i18n.translate("VoLTE Status").." : "..i18n.translate((uci:get("tooltest","gsm","status") or ""))
		elseif ip then
...
function action_gsm_tools()
	...
	if luci.http.formvalue("bcch") and luci.http.formvalue("bcch_save_btn") then
		...
		local gsm = uci:get_all("endpoint_mobile") or {}
...
function get_user_list()
	...
	local i18n = require "luci.i18n"
	local uci = require "luci.model.uci".cursor()
	local users = uci:get_all("user") or {}
	local user_list = {}

	for k,v in pairs(users) do
		if v['.type'] == "user" and v.name then
			local tmp={}
			tmp.name=v.name or ""
			if "admin" == v.group then
				tmp.group = i18n.translate("Administrator")
			elseif "operator" == v.group then
				tmp.group = i18n.translate("Operator")
			elseif "observer" == v.group then
				tmp.group = i18n.translate("Observer")
			else
				tmp.group = ""
			end
			tmp.expires=v.expires or i18n.translate("Always")
			tmp.description=v.description or ""
			tmp.status=v.status or "Enabled"
			tmp.edit=ds.build_url("admin","system","security","user",v.name,"edit")
			table.insert(user_list,tmp)
		end
	end
	return user_list
end

function get_edit_user_info(username)
	local uci = require "luci.model.uci".cursor()
	local user_info = uci:get_all("user",username) or {}
	local user_info_web = uci:get_all("user",username.."_web") or {}

	if next(user_info_web) then
		for k,v in pairs(user_info_web) do
			if "string" == type(v) then
				if v:match("view") then
					user_info[k.."_view"]=1
				end
				if v:match("edit") then
					user_info[k.."_edit"]=1
				end
			end
		end
...
function get_exist_user_namelist()
	local uci = require "luci.model.uci".cursor()
	local users = uci:get_all("user") or {}
	local exist_user="root&daemon&ftp&network&nobody&admin&api"
	for k,v in pairs(users) do
		if v['.type'] == "user" then
			exist_user=exist_user.."&"..v.name
		end
	end
	return exist_user
end

function user_add_edit(...)
	local ds = require "luci.dispatcher"
	local i18n = require "luci.i18n"
	local uci = require "luci.model.uci".cursor()

	if luci.http.formvalue("save") then
		local user = luci.http.formvaluetable("user") --user base setting
		local username = user.name
		local user_section = uci:get_all("user",username)
		if not user_section then
			uci:section("user","user",username)
		end
		uci:set("user",username,"name",username)
		if user.password and user.password_confirm and "" ~=user.password and user.password == user.password_confirm then
			uci:set("user",username,"password",user.password)
		end
		uci:set("user",username,"group",user.group)
		uci:set("user",username,"expires",user.expires)
		uci:set("user",username,"description",user.description)
		uci:set("user",username,"status",user.status)

		local web_access_raw = luci.http.formvaluetable("web") -- web access detail
		local web_access = {}
		for k,v in pairs(web_access_raw) do
			local page,action=k:match("([a-zA-Z0-9_]+)%.([viewdt]+)")
			if page and action then
				if not ("observer" == user.group and "edit" == action) then
					web_access[page] = web_access[page] and (web_access[page]..","..action) or action
				end
			end
		end

		local section_name = user.name.."_web"
		local user_web_section = uci:get_all("user",section_name)

		if not user_web_section then
			user_web_section = uci:section("user","web_access",section_name)
		else
			for k,v in pairs(user_web_section) do
				if not web_access[k] then
					uci:delete("user",section_name,k)
				end
			end
		end

		for k,v in pairs(web_access) do
			uci:set("user",section_name,k,v)
		end
	...
	elseif luci.http.formvalue("cancel") then
		...
		if arg[1] == "add" then
			luci.template.render("admin_system/user_edit",{
				title=i18n.translate("System / User Manager / New User"),
				action="add",
				exist_user=get_exist_user_namelist(),
			})
			return
		else
			name = arg[1]
			luci.template.render("admin_system/user_edit",{
				title=i18n.translate("System / User Manager / Edit User"),
				action="edit",
				user=get_edit_user_info(name),
			})
			return
		end
	end
	local user_list = get_user_list()
	local user_add_url = ds.build_url("admin","system","security","user","add")
	luci.template.render("admin_system/security",{user_list=user_list,user_add_url=user_add_url})
end
function security()
	local ds = require "luci.dispatcher"
	local uci = require "luci.model.uci".cursor()
	...
	if "save" == luci.http.formvalue("action") then
		...
		local sess = dsp.context.authsession
		...
		local msg = ""
		if "root" ~= user and current_user == user and luci.sys.user.checkpasswd(user,oldpw) then
			...
			if v1 and v2 and #v1 > 0 and #v2 > 0 then
				if v1 == v2 then
					if luci.sys.user.setpasswd(user, v1) == 0 then
						...
						if "admin" ~= user then
							local pass_line = util.exec("cat /etc/config/user | grep \"user \'"..user.."\'\" -n -A 6 | grep \"option password\"")
							pass_line = pass_line:match("^%d+")
							if not pass_line then
								pass_line = util.exec("cat /etc/config/user | grep \"user \'"..user.."\'\" -n")
							end
							v1=string.gsub(v1,"[%~%`%!%@%#%$%%%^%&%*%(%)%_%+%-%=%{%}%|%[%]%\%:\;\"\'%<%>%?%,%.%/]","\\%0")
							util.exec("sed -i \""..pass_line.."s/.*/	option password '"..v1.."'/\" /etc/config/user")
						end
						os.execute("grep \""..user.."\" /tmp/luci-sessions/* -l | xargs rm -rf")
					else
		...
		luci.template.render("admin_system/security",{message = msg,user_list=user_list,user_add_url=user_add_url})
	else
		local action_target = luci.http.formvaluetable("Action")
		if next(action_target) then
			local action,name
			for k,v in pairs(action_target) do
				action,name = k:match("([EnabledDiset]+)%.([a-zA-Z0-9_]+)%.[xy]")
				if action and name then
					break
				end
			end
			if ("Enabled" == action or "Disabled" == action) and name then
				uci:set("user",name,"status",action == "Enabled" and "Enabled" or "Disabled")
				uci:save("user")
			elseif "Delete" == action and name then
				uci:delete("user",name)
				uci:delete("user",name.."_web")
				uci:save("user")
			end
			user_list = get_user_list() -- refresh user list
		end
		luci.template.render("admin_system/security",{user_list=user_list,user_add_url=user_add_url})
	end
end
...
function webcmd()
	...
	if cmd then
		...
		elseif cmd:match("^%s*button%s+pressed%s+history%s*$") then
			fscmd = false
			r = fs.readfile("/etc/log/button_log") or "Button pressed history is empty !"
		...
		elseif cmd:match("^%s*ated%s+%-i%s+ra0%s*$") then
			if "" == util.exec("pgrep 'ated'") then
				os.execute("ated -i ra0 && sleep 1")
				if "" ~= util.exec("pgrep 'ated'") then
					r = "ated start succ"
				else
					r = "ated start fail"
				...
				r="ated has been executed before"
			end
			fscmd=false
		elseif cmd:match("^%s*ated%s+stop%s*$") then
			if "" ~= util.exec("pgrep 'ated'") then
				os.execute("killall ated && sleep 1")
				if "" == util.exec("pgrep 'ated'") then
					r = "ated stop succ"
				else
					r = "ated stop fail"
...
function patch_active(id)
	...
	local uci = require "luci.model.uci".cursor()
	local req_from = (luci.http.getenv("REMOTE_ADDR") or "") ..":".. (luci.http.getenv("REMOTE_PORT") or "")
	local log_str = req_from.." | ".."ActivePatch /admin/system/backup_upgrade/active_patch/"..id
	log.web_operation_log("Info",log_str)
	for i=1,id do
...
function patch_deactive(id,del_flag)
	local fs = require("luci.fs")
	local nixio = require "nixio"
	local uci = require "luci.model.uci".cursor()
	...
	if not del_flag then
		local req_from = (luci.http.getenv("REMOTE_ADDR") or "") ..":".. (luci.http.getenv("REMOTE_PORT") or "")
		local log_str = req_from.." | ".."DeactivePatch /admin/system/backup_upgrade/deactive_patch/"..id
		log.web_operation_log("Info",log_str)
	end
...
function patch_delete(id)
	...
	local max = tonumber(luci.util.exec("ls /usr/lib/lua/patch | wc -l") or 100)
	local req_from = (luci.http.getenv("REMOTE_ADDR") or "") ..":".. (luci.http.getenv("REMOTE_PORT") or "")
	local log_str = req_from.." | ".."DeletePatch /admin/system/backup_upgrade/delete_patch/"..id
	log.web_operation_log("Info",log_str)

	if patch_deactive(id,true) then
		local rm_list = ""
		for i=id,max do
			rm_list=rm_list..i.." "
			uci:delete("patch",i)
...
function backup()
	...
	if bkp_list.system then
		...
		cfg_backup_cmd = cfg_backup_cmd.."find /etc/config/user -type f;"
		...
		function(meta, chunk, eof)
			...
			if eof and fp then
				...
				if meta and "archive" == meta.name then
					...
					os.execute("cp /etc/config/user /etc/config/user_backup")
				...
				elseif meta and "software" == meta.name and "system" == upgrade_type then
					...
					web_rpc_syn(600000, "provision", "cli-download", "firmware", destfile, web_rpc_syn_cb)
...
function ctl_xml()
	...
	if file then
		...
		elseif action ~= "read" and action ~= "write" and action ~= "restore" then
			info["info"] = "Unknown action!"
			info["ret"] = "false"
			luci.http.prepare_content("application/json")
			luci.http.write_json(info)
			return
		elseif action == "write" and (not content or content == "")then
			info["info"] = "No Acceptable content!"
			info["ret"] = "false"
			luci.http.prepare_content("application/json")
			luci.http.write_json(info)
			return
