...
function index()
	...
	if drv_str == "rt2860v2_ap" or drv_str == "rt2860v2_sta" then
		entry({"admin","network","wlan"}, alias("admin","network","wlan","wlan_config"), _("WLAN(AP)"),20)

		local wifi_mode = uci:get("wireless","wifi0","mode")
		if wifi_mode == "sta" then
			entry({"admin","network","wlan","wlan_config"},template("admin_network/wlan_sta"))
			entry({"admin","network","wifi_check"},call("action_wifi_check"))
		else
...
function action_wifi_check_config(ssid,encryption,key)
	local fs = require "luci.fs"
	if ssid and encryption and (encryption == "none" or (encryption ~= "none" and key)) then
		local cmd_str="#!/bin/sh\n"

		if encryption == "none" then
			cmd_str = cmd_str.."iwpriv ra0 set AuthMode=OPEN\n"
			cmd_str = cmd_str.."iwpriv ra0 set EncrypType=NONE\n"
			cmd_str = cmd_str.."iwpriv ra0 set SSID=\""..ssid.."\"\n"
		elseif encryption == "wep" then
			cmd_str = cmd_str.."iwpriv ra0 set AuthMode=WEPAUTO\n"
			cmd_str = cmd_str.."iwpriv ra0 set EncrypType=WEP\n"
			cmd_str = cmd_str.."iwpriv ra0 set DefaultKeyID=1\n"
			cmd_str = cmd_str.."iwpriv ra0 set Key1=\""..key.."\"\n"
			cmd_str = cmd_str.."iwpriv ra0 set SSID=\""..ssid.."\"\n"
		elseif encryption == "psk" then
			cmd_str = cmd_str.."iwpriv ra0 set AuthMode=WPAPSK\n"
			cmd_str = cmd_str.."iwpriv ra0 set EncrypType=AES\n"
			cmd_str = cmd_str.."iwpriv ra0 set WPAPSK="..key.."\n"
			cmd_str = cmd_str.."iwpriv ra0 set SSID=\""..ssid.."\"\n"
		elseif encryption == "psk2" then
			cmd_str = cmd_str.."iwpriv ra0 set AuthMode=WPA2PSK\n"
			cmd_str = cmd_str.."iwpriv ra0 set EncrypType=AES\n"
			cmd_str = cmd_str.."iwpriv ra0 set WPAPSK="..key.."\n"
			cmd_str = cmd_str.."iwpriv ra0 set SSID=\""..ssid.."\"\n"
		end

		fs.writefile("/tmp/sta_test.sh", cmd_str)
		os.execute("sh /tmp/sta_test.sh")
	end
end

function action_wifi_check()
	local util = require "luci.util"
	local fs = require "luci.fs"
	local exe = require "os".execute
	local tmp_uci = require "luci.model.uci".cursor("/tmp/config")
	local action_type = luci.http.formvalue("action_type")

	if action_type == "test_start" then
		local ssid = luci.http.formvalue("ssid")
		local enc = luci.http.formvalue("encryption")
		local key = luci.http.formvalue("key")

		if fs.access("/tmp/sta_check_flag") or not ssid or not enc then
			return
		end

		exe("rm /tmp/sta_check_state -rf; touch /tmp/sta_check_flag; cp /etc/config/wireless /tmp/config/tmp_wireless;")

		if enc == "wep" then
			local wep = luci.http.formvalue("wep") or "64bit"
			if wep == "64bit" then
				key = util.exec("echo -n '"..(key or "").."' | hexdump -e '5/1 \"%02x\"'")
			else
				key = util.exec("echo -n '"..(key or "").."' | hexdump -e '13/1 \"%02x\"'")
			end
		end
		action_wifi_check_config(ssid,enc,key)

		local num = 0
		while num < 15 do
			exe("sleep 1")
			local ret = util.exec("iwpriv ra0 connStatus"):match("(Connected)")
			if ret then
				break
			end
			num = num + 1
		...
		if num == 15 then
			fs.writefile("/tmp/sta_check_state","failed")
		else
			fs.writefile("/tmp/sta_check_state","success")
		end

		ssid = tmp_uci:get("tmp_wireless","wifi0","ssid")
		enc = tmp_uci:get("tmp_wireless","wifi0","encryption")
		key = tmp_uci:get("tmp_wireless","wifi0","key")
		action_wifi_check_config(ssid,enc,key)

		exe("rm /tmp/sta_check_flag /tmp/config/tmp_wireless")

		return
	elseif action_type == "test_ready" then
		local status = fs.access("/tmp/sta_check_flag") and "1" or "0"
		luci.http.prepare_content("application/json")
		luci.http.write_json({status=status})
		return
	elseif action_type == "status" then
		local status
		if not fs.access("tmp/sta_check_state") then
			status = "2"
		else
			local ret = util.exec("cat /tmp/sta_check_state"):match("(success)")
			status = ret and "0" or "1"
		end
		luci.http.prepare_content("application/json")
		luci.http.write_json({status=status})
	end
