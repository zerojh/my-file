...
function index()
	...
	entry({"admin","network","tfcard"},call("action_tfcard"),_("Mobile Storage"),160).leaf = true
end
...
function action_tfcard()
	...
	local file_content = {}
	--@ default u-disk dir
	local cur_dir = "/mnt/mmcblk0p1"
	local tmp_file = "/mnt/mmcblk0p1"
	local file_name = ""
	local upload_file
	...
	elseif luci.http.formvalue("change_dir") then
		...
		if not cur_dir:match("^/mnt/mmcblk0p1") then
			cur_dir = "/mmcblk0p1"
		end
	--@ goto new directory
	...
	elseif luci.http.formvalue("safe_popup") then
		util.exec("umount /mnt/mmcblk0p1")
		cur_dir = "/mnt/mmcblk0p1"
	else
		--@ first in ,do nothing
	end
	...
	if refresh_flag then
		...
		local total_size,available_size = parse_tfcard_size("df -h | grep /dev/mmcblk0p1")
		local _,max_size = parse_tfcard_size("df | grep /dev/mmcblk0p1")
		max_size = (max_size or 0) * 1024
		
		luci.template.render("admin_network/tfcard",{
			cur_dir = cur_dir,
