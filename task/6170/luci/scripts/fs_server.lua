...
function pstn_status(reset_flag)
	...
	for k,v in pairs(ret_tb) do
		...
		elseif ("GSM1" == v.type or "GSM" == v.type) and not gsm_status.disabled then
			...
			local con = ESL.ESLconnection("127.0.0.1","8021","ClueCon")
			if con:connected() ~= 1 then
				v.phonenumber="Unknown"
				v.carrier="Unknown"
			else
				local ret_str = con:api("gsm dump 1"):getBody()
				v.phonenumber=ret_str:match("phone_num%s*=%s*([0-9A-Z\+\*\#]+)") or "Unknown"
				v.carrier=ret_str:match("opname%s*=%s*(.+)\nphone_num") or "Unknown"
				con:disconnect()
			end
	...
	if not cmd then
		...
		if not str:match("^-ERR") then
			...
			for k,v in pairs(tmp_tb) do
				if v:match("chan_ready%s*=%s*(%d+)\n") then
					local temp = {}

					temp.slot = v:match("slot%s*=%s*([0-9]+)\n")
					temp.type = v:match("boardtype%s*=%s*([0-9A-Z]+)") or "Unknown"
					temp.carrier = v:match("opname%s*=%s*(.+)\nphone_num") or "Unknown"
					temp.interface_name = v:match("interface_name%s*=%s*([a-zA-Z0-9_%-]+)\n") or "Unknown"
					temp.name = "SIM "..temp.slot.." / "..(temp.interface_name:match("%-([0-9a-zA-Z]+)") or "Unknown") --.." / "..i18n.translate(temp.carrier)
					if 1 ~= tonumber(v:match("chan_ready%s*=%s*(%d+)\n")) then
						temp.status = "no_device"
					else
						if "SIMPIN_READY" ~= v:match("simpin_state%s*=%s*([a-zA-Z_]+)\n") then
							temp.status = "no_card"
						else
							if "0" ~= v:match("not_registered%s*=%s*(%d+)\n") then
								temp.status = "no_registered"
							else
...
function get_sms()
	...
	send_tb,err = sqlite.sqlite3_execute(sms_db_dir,"select * from sms_sendover")
	
	ret_tb,err = sqlite.sqlite3_execute(sms_db_dir,"select * from sms_sending")
	for k,v in pairs(ret_tb) do
...
function get_wifi_list(param)
	...
	if param and param == "refresh" then
		...
		if drv_str == "rt2860v2_ap" then
			...
			if drv_str == "rt2860v2_ap" then
				util.exec("iwpriv ra0 set SiteSurvey=")
			end
		...
		elseif drv_str == "rt2860v2_sta" or drv_str == "rt2x00" then
			--@ old driver
			local ret_str
			local tmp_tb
			local status

			if drv_str == "rt2860v2_sta" then
			...
			if drv_str == "rt2860v2_sta" then
				ret_str = util.exec("iwlist ra0 scanning|sed '1d'|tr '\n' ','|tr -d ' '|sed 's/,Cell/\\nCell/g'")
			else
				ret_str = util.exec("iwlist wlan0 scanning|sed '1d'|tr '\n' ','|tr -d ' '|sed 's/,Cell/\\nCell/g'")
			end
			tmp_tb = luci.util.split(ret_str,"\n")
