...
function get_vpn_info()
	...
	if not fs.access("/tmp/config/vpnselect") then
		util.exec("touch /tmp/config/vpnselect")
		if not uci_tmp:get("vpnselect","vpnselect") then
			uci_tmp:section("vpnselect","select","vpnselect")
			uci_tmp:save("vpnselect")
			uci_tmp:commit("vpnselect")
		end
	end

	local vpn_type = uci:get("vpnselect","vpnselect","vpntype") or uci_tmp:get("vpnselect","vpnselect","vpntype")
	if not vpn_type then
		...
		uci_tmp:set("vpnselect","vpnselect","vpntype",vpn_type)
		uci_tmp:save("vpnselect")
		uci_tmp:commit("vpnselect")
	end
	...
	if vpn_type == "l2tp" then
		vpn_info.type = "L2TP"
		iface_str = util.exec("ifconfig | grep 'ppp1701'")
		log_str = util.exec("tail -n 1 /ramlog/l2tpc_log | grep '^login:'")
		if iface_str == "" or log_str == "" then
	...
	elseif vpn_type == "pptp" then
		vpn_info.type = "PPTP"
		iface_str = util.exec("ifconfig | grep 'ppp1723'")
	...
	elseif vpn_type == "openvpn" then
		...
		iface_str = util.exec("ifconfig | grep 'tun0'")
