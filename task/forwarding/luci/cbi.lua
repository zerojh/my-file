...
FileId:
$Id: cbi.lua 7810 2011-10-28 15:15:27Z jow $
...
function Map.__init__(self, config, ...)
	...
	self.flow = {}
	self.addnew = false
	self.new_section = nil
	self.uci = uci.cursor()
	self.save = true
	self.currsection = nil
...
function Map.gen_redirect(self,param)
	local addr,exparam
	if string.find(param,";") then
		addr,exparam = param:match("^([a-zA-Z0-9_%-]+);(.+)")
	else
		addr = param
		exparam = ""
	end
	local mod,submod,cfg,action = addr:match("(%w+)-(%w+)-(%w+)-(%w+)")

	if submod and submod:match("^fx[so]") and "profile" == mod then
		return luci.dispatcher.build_url("admin",mod,"fxso",submod,cfg,action,exparam)
	else
		return luci.dispatcher.build_url("admin",mod,submod,submod,cfg,action,exparam)
	end
end

function Map.insertprofile(self,param)
	local profile,exparam = param:match("([a-z_]+)/(.+)")
	local module,submodule = profile:match("(%w+)_(%w+)$")
	local map = Map(profile)
	local section = map:section(TypedSection,submodule)
	local created = TypedSection.create(section,nil)
	map.uci:save(profile)
	luci.http.redirect(luci.dispatcher.build_url(
		"admin",module,submodule,submodule,created,"add",exparam
	))
	return
end
...
function Map.parse(self, readinput, ...)
	...
	if self:formvalue("cbi.insert.new") then
		self.currsection.inserting = true
	end
	if not self:formvalue("cbi.cancel") then
		Node.parse(self, ...)
	end
	if self.save then
		self:_run_hooks("on_save", "on_before_save")
		for i, config in ipairs(self.parsechain) do
	        --if self:formvalue("cbi.cancel") and self.addnew and self.new_section then
	        --    self.uci:delete(config,self.new_section)
	        --end
		...
		if self.redirect and (self:formvalue("cbi.cancel") or self:formvalue("cbi.save") or self:formvalue("cbi.insert.new")) then
			local cbi_submit_redirect = self:formvalue("cbi.insert.new")
			if not cbi_submit_redirect then
				luci.http.redirect(self.redirect)
			else
				self:insertprofile(cbi_submit_redirect)
			end
		end
...
--@ harlan for ivr
--[[
IvrSection - A fixed configuration section defined by its name
]]--
IvrSection = class(AbstractSection)

function IvrSection.__init__(self, map, section, stype, ...)
	AbstractSection.__init__(self, map, stype, ...)

	-- Defaults
	self.addremove = false
	self.template = "cbi/ivrsection"
	self.section = section
	self.notitle = false
end

function IvrSection.parse(self, novld)
	local s = self.section
	local active = self:cfgvalue(s)
...
function AbstractValue.add_error(self, section, type, msg)
	if true == self.section.inserting then
		return
	end
	self.error = self.error or { }
	--self.error[section] = msg or type
	self.error[section] = self.error[section] or {}
	local err_msg = {}
	err_msg["tag"] = type;
	err_msg["msg"] = msg;

	self.error[section] = err_msg
	self.section.error = self.section.error or { }
	self.section.error[section] = self.section.error[section] or { }
	table.insert(self.section.error[section], err_msg)

	if type == "invalid" then
		self.tag_invalid[section] = true
		self.section.tag_invalid[section] = true
	elseif type == "missing" then
		self.tag_missing[section] = true
		self.section.tag_missing[section] = true
	end
	...
	if fvalue and #fvalue > 0 then -- If we have a form value, write it to UCI
		...
		if not fvalue and not novld then
			...
			if type(raw_value) == "table" then
				for _,v in pairs(raw_value) do
					self:add_error(section,"invalid",string.format("%-16s: %s",tostring(self.title),v))
				end
			else	
				self:add_error(section,"invalid",string.format("%-16s: %s",tostring(self.title),raw_value))
			end
		end
		...
		elseif (cvalue ~= fvalue or (cvalue == fvalue and not self.rmempty and not fvalue)) and not novld then
			-- trigger validator with nil value to get custom user error msg.
			local ret, val_err = self:validate(nil, section)
			--self:add_error(section, "missing", val_err)
			if not ret then
				self:add_error(section,"missing",self.title)
			end
		end
	end
end
	...
	for i, deps in ipairs({...}) do
		for k,v in pairs(deps) do
			if "table" == type(v) then
				self.subdeps[#self.subdeps + 1] = {add = "-"..key, deps=v}
			else
...
DynListValue = class(AbstractValue)
...
function DynListValue.reset_values(self)
	self.keylist = {}
	self.vallist = {}
end

function DynListValue.value(self, key, val, ...)
...
function DynListValue.cfgvalue(self, section)
...
function DynListValue.write(self, section, value)
	local t = { }
...
function CallForwarding.cfgvalue(self, section)
	local tmp_value = AbstractValue.cfgvalue(self, section)
	local value

	if type(tmp_value) == "string" then
		local x
		local t = { }
		for x in tmp_value:gmatch("%S+") do
			if #x > 0 then
				t[#t+1] = x
			end
		end
	...
	if tmp_value and next(tmp_value) then
		value = {}
		for k,v in pairs(tmp_value) do
			...
			if not tmp_tb[1] or not tmp_tb[2] or not tmp_tb[3]then
				tmp_tb[3] = ""
				tmp_tb[1],tmp_tb[2] = v:match("([^:]*)::([^:]*)")
				if not tmp_tb[2] then
					tmp_tb[1] = v
					tmp_tb[2] = ""
				end
			end
			table.insert(value,tmp_tb)
		end
	end
