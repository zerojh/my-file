...
function index()
	...
	entry({"admin","callcontrol","forwardgroup"},call("forwardgroup"),"Call Forward Group",10)
	entry({"admin","callcontrol","forwardgroup","forwardgroup"}, cbi("admin_callcontrol/forwardgroup_edit"),nil,10).leaf = true
end
...
function action_sms_route()
	...
	local cnt = 0
	local profile = uci:get_all("profile_smsroute") or {}
...
function routegroup()
	...
	local addnewable = true
	local cnt = 0
	local routegrp = uci:get_all("endpoint_routegroup") or {}
						...
						for k,v in pairs(uci:get_all("endpoint_fxso") or {}) do
							...
							if v.index == tmp_index and "fxo" == v[".type"] and dtype == "FXO" then
								if interface:match("1O") then
									...
									dest = "FXO / "..i18n.translate("Port").." "..tmp_port
								end
								break
							end
						end
...
function forwardgroup()
	...
	local del_target = luci.http.formvaluetable("Delete")
	if next(del_target) then
		uci:delete_section(del_target,"forward.endpoint")
	end

	if luci.http.formvalue("New") then
		local created = uci:section("endpoint_forwardgroup","group")
		uci:save("endpoint_forwardgroup")
		luci.http.redirect(ds.build_url("admin","callcontrol","forwardgroup","forwardgroup",created,"add"))
		return
	end

	local destination_tb = {}
		...
		for k,v in pairs(uci:get_all("endpoint_fxso") or {}) do
			if "fxs" == v['.type'] and v.index and "Enabled" == v.status then
				if luci.version.license.fxs > 1 then
					destination_tb[v.number_1] = tostring(i18n.translate("FXS Extension").." "..tonumber(2*(v.index-1)))
					destination_tb[v.number_2] = tostring(i18n.translate("FXS Extension").." "..tonumber(2*(v.index-1)+1))
				else
					destination_tb[v.number_1] = tostring(i18n.translate("FXS Extension"))
				end
			elseif "fxo" == v['.type'] and v.index and "Enabled" == v.status then
				if luci.version.license.fxo > 1 then
					destination_tb["FXO/"..v.index.."/1"] = tostring(i18n.translate("FXO Trunk").." "..tonumber(2*(v.index-1)))
					destination_tb["FXO/"..v.index.."/2"] = tostring(i18n.translate("FXO Trunk").." "..tonumber(2*(v.index-1)+1))
				else
					destination_tb["FXO/"..v.index.."/2"] = tostring(i18n.translate("FXO Trunk"))
				end
			end
		end
	end
	for k,v in pairs(uci:get_all("endpoint_sipphone") or {}) do
		if v.index and v.name and "Enabled" == v.status and v.user then
			destination_tb[v.user] = tostring(i18n.translate("SIP Extension").."-< "..v.name.." >")
		end
	end
	for k,v in pairs(uci:get_all("endpoint_mobile") or {}) do
		if v.index and "Enabled" == v.status and v.slot_type and v.slot_type:match("GSM$") and v.name then
			destination_tb["gsmopen/"..v.slot_type] = tostring(i18n.translate("GSM Trunk"))
		elseif v.index and tonumber(v.index) == i and "Enabled" == v.status and v.slot_type and v.slot_type:match("CDMA$") and v.name then
			destination_tb["gsmopen/"..v.slot_type] = tostring(i18n.translate("CDMA Trunk"))
		end
	end
	for k,v in pairs(uci:get_all("endpoint_siptrunk") or {}) do
		if v.index and "Enabled" == v.status and v.index and v.name and v.profile then
			destination_tb["SIPT-"..v.profile.."_"..v.index] = tostring(i18n.translate("SIP Trunk").." "..v.name)
		end
	end

	local th = {"Index","Name","Call Forward Destination"}
	local colgroup = {"5%","10%","76%","7%"}
	local content = {}
	local edit = {}
	local delchk = {}
	local uci_cfg = {}
	local addnewable = true
	local cnt = 0
	local forwardgrp = uci:get_all("endpoint_forwardgroup") or {}
	for i=1,MAX_FORWARD_GRP do
		for k,v in pairs(forwardgrp) do
			if v.index and v.name and i == tonumber(v.index) then
				cnt = cnt + 1
				local td = {}
				td[1] = v.index
				td[2] = v.name
				td[3] = {}
				for _,val in ipairs(v.destination or {}) do
					...
					if dest and destination_tb[dest] then
						if number then
							table.insert(td[3], destination_tb[dest].."-< "..number.." >")
						else
							table.insert(td[3], destination_tb[dest])
						end
					end
				end
				...
				break
			elseif not v.index or not v.name then
				uci:delete("endpoint_forwardgroup",k)
				--uci:save("endpoint_forwardgroup")
			end
		end
	end
	if MAX_FORWARD_GRP == cnt then
		addnewable = false
	end
	luci.template.render("cbi/configlist",{
		title = i18n.translate("Call Control / Call Forward Group"),
		colgroup = colgroup,
		split_col = 4,
		classname = "paddingtight",
		th = th,
