...
if session:ready() then
	...
	if (bridge_str:match("sofia/") or bridge_str:match("user/")) and from:match("^sofia") then
		...
		if reg_info[bridge_name] and "caller" ~= reg_info[bridge_name]["from_username"] then
			session:execute("set","effective_caller_id_name="..reg_info[bridge_name]["from_username"])
			session:execute("set","effective_caller_id_number="..reg_info[bridge_name]["from_username"])
		else
			if session:getVariable("effective_caller_id_number") then
				session:execute("export","nolocal:sip_contact_user=${effective_caller_id_number}")
			else
				session:execute("export","nolocal:sip_contact_user=${caller_id_number}")
			end
		end
