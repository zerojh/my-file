#!/bin/sh
	...
	build_opkg_script()
	{
		...
		if [ "freeswitch" == ${bin} ]; then
			cp $servicefile_fs $srvdir/$bin
		else
	...
	build_lua_web()
	{
		case $1 in
			opkg)
				...
				local tmplib=$tmpname/usr/lib/lua
				local tmpwww=$tmpname/www
				local tmpctrl=$tmpname/CONTROL
				local tmpconf=$tmpname/etc
	...
	build_cloud()
	{
		case $1 in
			opkg)
				...
				local tmpctrl=$tmpname/CONTROL
				local tmpsbin=$tmpname/usr/sbin
				local tmpsrv=$tmpname/etc/init.d
				...
				#准备安装脚本
				build_opkg_script $tmpctrl $tmpsrv cloud 99 99 ""
cat >>$tmpctrl/preinst << EOF 
mkdir -p /tmp/cloud_config
if [ -f /etc/config/cloud ]; then
cp /etc/config/cloud /tmp/cloud_config/
fi
EOF


cat >>$tmpctrl/postinst << EOF 
if [ -f /tmp/cloud_config/cloud ]; then
cp /tmp/cloud_config/cloud  /etc/config
fi
if [ -f /etc/init.d/cloud ]; then
/etc/init.d/cloud enable
fi
if [ -f /etc/init.d/cloud ]; then
/etc/init.d/cloud start
fi
rm -rf /tmp/cloud_config
EOF
				
				cp $bindir/cloud/cloud $tmpsbin
				cp $bindir/cloud/remoted $tmpsbin
				cp $bindir/cloud/etc $tmpname -r
	...
	build_sounds()
	{
		case $1 in
			opkg)
				...
				local tmpctrl=$tmpname/CONTROL
				local tmpconf=$tmpname/etc/freeswitch/sounds
	...
	build_dsp()
	{
		case $1 in
			opkg)
				...
				local tmpctrl=$tmpname/CONTROL
				local tmpconf1=$tmpname/bin/
				local tmpconf2=$tmpname/tmp
				...
				#准备安装脚本
				cp $dspdir/dsp $tmpconf1
				#cp $dspdir/ralink_pcm.ko $tmpconf2
	...
	build_freeswitch()
	{
		case $1 in
			...
			opkg)
				...
				build_opkg_control "$tmpctrl/control" \
				"freeswitch" \
				"`get_pc_var $bindir/freeswitch $pcname Version`-`date +%y%m%d%H%M`" \
				"misc" \
				"optional" \
				"$buildarch" \
				"Freeswitch" \
				...
				#准备安装脚本
				build_opkg_script $tmpctrl 	$tmpsrv freeswitch 99 16 '-base \/etc\/freeswitch -conf \/etc\/freeswitch\/conf -db \/tmp\/fsdb -log \/var\/log -mod \/lib\/fsmod -nf -np -nc -nocal -product UBG1000'

				mkdir -p $tmpname/bin
				cp $bindir/freeswitch/bin/freeswitch $tmpname/bin
				cp $bindir/freeswitch/bin/fs_cli $tmpname/bin
				cp $syslibdir/bin/audioconvert $tmpname/bin

				#lib放到lib目录
				mkdir -p $tmpname/lib
				cp $bindir/freeswitch/lib/libfreeswitch.so.1.0.0 $tmpname/lib
				cp $bindir/freeswitch/lib/libc300dsp.so.0.0.0 $tmpname/lib
				cp $bindir/freeswitch/lib/libfreetdm.so.1.0.0 $tmpname/lib
				cp $bindir/freeswitch/lib/libtrans.so.0.0.0 $tmpname/lib
				cp $bindir/freeswitch/lib/libc300cap.so.0.0.0 $tmpname/lib

				#mod放到
				mkdir -p $tmpname/lib/fsmod
				cp $bindir/freeswitch/mod/*.so $tmpname/lib/fsmod

				#需要保存的配置,都放到etc/freeswitch目录下
				mkdir -p $tmpname/etc/freeswitch/conf
				mkdir -p $tmpname/etc/freeswitch/grammar
				mkdir -p $tmpname/etc/freeswitch/scripts
				mkdir -p $tmpname/etc/freeswitch/sounds
				cp -R $confdir/etc/freeswitch/* $tmpname/etc/freeswitch/
				...
				#add corn check app
				mkdir -p $tmpname/usr/bin
				cp $confdir/etc/cron_app.sh $tmpname/usr/bin
				chmod +x /$tmpname/usr/bin/cron_app.sh
				cp $confdir/etc/cron_system.sh $tmpname/usr/bin
				chmod +x /$tmpname/usr/bin/cron_system.sh
...
if [ -f /etc/crontabs/root ]; then
	sed -i '/cron_app/d' /etc/crontabs/root
	echo '*/1 * * * * /usr/bin/cron_app.sh' >> /etc/crontabs/root
else
	echo '*/1 * * * * /usr/bin/cron_app.sh' > /etc/crontabs/root
fi

    cat >/etc/hotplug.d/iface/01-wan2up << EOF1
[ ifup = "\\\$ACTION" ] && [ \\\$INTERFACE == "wan2" ] && {
        if [ -f /etc/freeswitch/scripts/lte_if_up.lua ]; then
            lua /etc/freeswitch/scripts/lte_if_up.lua
        fi
}
EOF1
    chmod +x /etc/hotplug.d/iface/01-wan2up
EOF
...
EOF
	...
	build_filesystem()
	{
		case $1 in
			opkg)
				local curpwd=`pwd`
				local tmpsrc="filesystem git `get_git_ver`"
				local tmpname=$tmpdir/opkg/filesystem
				local tmpctrl=$tmpname/CONTROL
				local tmpconf=$tmpname/tmp/firmware_upgrading_temp/filesystem/
				...
				#准备安装脚本
				cp $filesystemdir/extension $tmpconf
				cp $filesystemdir/raeth_ubg.ko $tmpconf
				cp $filesystemdir/postinst $tmpctrl
	...
	build_sub()
	{
		...
		case $sub in
			...
			dsp)
				...
				;;
			filesystem)
				echo "------>enter dir $filesystemdir"
				cd $filesystemdir
				build_filesystem $action
				cd ../..
				echo "<------leave dir $filesystemdir"
				;;
			*)
	...
	build_ld()
	{
		...
		if [ -d ${packagedir}_${ver}_${lang} ] ; then
			rm -rf ${packagedir}_${ver}_${lang}
		fi
		mv ${packagedir} ${packagedir}_${ver}_${lang}
    ...
    ARGS=`getopt -a -o a:c:p:v:r:l:hwq -l cpu:,action:,product:,version:,rely:,list:,lang:,without-syslib,quiet,help,nostrip,gdb,noconfigure -- "$@"`
	...
	dspdir=$basedir/dsp
	filesystemdir=$basedir/filesystem
	...
	servicefile_fs=$basedir/package/service_fs
	...
	case ${cpu} in
		...
		mipsel)
			...
			buildarch=mipsel
			export STAGING_DIR=/home/samba/openwrt-ubg1000/staging_dir
			...
			echo "PKG_CONFIG_PATH=$PKG_CONFIG_PATH, libdir=$libdir"
			export ARCH=mips
			export VAPI_FLAGS=-DPRODUCT_UBG1000
			export pbc_dest_dir=$libdir
			...
			#公共的cflags参数
			common_cflags="${gdb} -rdynamic -I$syslibdir/include/dpr -I$STAGING_SYS/include  -I$libdir/include -I$libdir/include/apr-1 -I$libdir/include/curl -I$libdir/include/dpr -DPRODUCT_UBG1000"
			common_cxxflags="-I$syslibdir/include/dpr -I$STAGING_SYS/include -I$libdir/include -I$libdir/include/apr-1 -I$libdir/include/curl -I$libdir/include/dpr -DPRODUCT_UBG1000"
