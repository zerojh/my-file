...
function get_leasetime(value)
	if not value or value == "infinite" then
		return translate("Permanent")
	end

	if value:match("(%d+)h") then
		return value
	elseif value:match("(%d+)m") then
		local min = tonumber(value:match("(%d+)m"))
		local hour = math.modf(min / 60)
		min = min % 60
		return hour.."h "..min.."m"
	elseif value:match("(%d+)s") then
		local sec = tonumber(value:match("(%d+)s"))
...
function get_config_option_value(cfg_name,section,option,value)
	...
	elseif "leasetime" == option then
		return get_leasetime(value)
