...
function get_leasetime(value)
	...
	if not value or value == "infinite" then
		return i18n.translate("Permanent")
	end
...
function get_dhcp_info()
	...
	if "" == dnsmasq_running_conf or string.find(dnsmasq_running_conf,"no%-dhcp%-interface") then
		...
		if string.find(dnsmasq_running_conf,"dhcp%-range=lan") then
			dhcp.startaddr, dhcp.endaddr,dhcp.netmask,dhcp.expires=dnsmasq_running_conf:match("dhcp%-range=lan,([0-9%.]+),([0-9%.]+),([0-9%.]+),([0-9a-z]+)")
			...
			dhcp.expires=get_leasetime(dhcp.expires)
		else
			dhcp.startaddr = "-"
			dhcp.endaddr = "-"
			dhcp.expires = "-"
...
function get_lte_info()
	...
	local tmp = utl.exec("cat /etc/config/endpoint_mobile | grep lte_mode") or ""
	local config_mode = string.upper(tmp:match("lte_mode '([a-zA-Z0-9%-]+)'") or "unknown")
	...
	if "" == running_mode_detail then
		...
		lte_info.mode = config_mode.." / "..i18n.translate(running_mode).." / "..i18n.translate(running_mode_detail)
	end
