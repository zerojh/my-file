...
function sync_user_to_system()
	...
	for k,v in ipairs(passwd_file_user_tb) do
		if "root" ~= v and "daemon" ~= v and "ftp" ~= v and "network" ~= v and "nobody" ~= v and "admin" ~= v and "" ~= v then
	...
	for k,v in pairs(user_list) do
		if "user" == v[".type"] and v.name and "root" ~= v.name and "daemon" ~= v.name and "ftp" ~= v.name and "network" ~= v.name and "nobody" ~= v.name and "admin" ~= v.name then
...
function config_cron()
	local cron_tb = uci:get_all("cron") or {}
	local cron_public = ""
	local cron_optional = ""
	...
	for k,v in pairs(cron_tb) do
		if v[".name"] == "netmount" then
			cron_optional = cron_optional.."*/1 * * * * /usr/bin/netmount.sh &\n"
			exe("lua /usr/lib/lua/luci/scripts/netmount_sed.lua; sh /usr/bin/netmount.sh;")
		end
	end

	fs.writefile("/tmp/root",cron_public..cron_optional)
	exe("cp /tmp/root /etc/crontabs/root")
end
...
function load_scripts(param)
	...
	if param.cron or param.restore or param.upgrade then
		applyed_list = add_to_list(applyed_list,"Cron")
		fs.writefile("/tmp/fs-apply-status","Applying="..applyed_list)
