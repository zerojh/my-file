...
function check_pstn_status()
	...
	if "1T1S1O" == string.upper(uci:get("system","main","interface") or "") then
		os.execute("sed -i 's/-product UC100-[0-9a-zA-Z]*/-product UC100-1T1S1O/g' /etc/init.d/freeswitch")
	elseif "1G1S" == string.upper(uci:get("system","main","interface") or "") then
		os.execute("sed -i 's/-product UC100-[0-9a-zA-Z]*/-product UC100-1G1S/g' /etc/init.d/freeswitch")
	elseif "1S1O" == string.upper(uci:get("system","main","interface") or "") then
		os.execute("sed -i 's/-product UC100-[0-9a-zA-Z]*/-product UC100-1S1O/g' /etc/init.d/freeswitch")
	elseif "1S" == string.upper(uci:get("system","main","interface") or "") then
		os.execute("sed -i 's/-product UC100-[0-9a-zA-Z]*/-product UC100-1S/g' /etc/init.d/freeswitch")
	elseif "2O" == string.upper(uci:get("system","main","interface") or "") then
		os.execute("sed -i 's/-product UC100-[0-9a-zA-Z]*/-product UC100-2O/g' /etc/init.d/freeswitch")
	elseif "1V1S1O" == string.upper(uci:get("system","main","interface") or "") then
		os.execute("sed -i 's/-product UC100-[0-9a-zA-Z]*/-product UC100-1V1S1O/g' /etc/init.d/freeswitch")
	elseif "1V1S" == string.upper(uci:get("system","main","interface") or "") then
		os.execute("sed -i 's/-product UC100-[0-9a-zA-Z]*/-product UC100-1V1S/g' /etc/init.d/freeswitch")
	elseif "1V1O" == string.upper(uci:get("system","main","interface") or "") then
		os.execute("sed -i 's/-product UC100-[0-9a-zA-Z]*/-product UC100-1V1O/g' /etc/init.d/freeswitch")
	else
		os.execute("sed -i 's/-product UC100-[0-9a-zA-Z]*/-product UC100-1G1S1O/g' /etc/init.d/freeswitch")
	end
end
...
function device_info_init()
	...
	uci:commit("ddns")
	-- 1、only 1g1s1o/1t1s1o module at the first time, interface info store in uci system.main.interface , product value in the license is uc100
	...
	local current_hostname = string.upper(uci:get("system","main","hostname") or "UC100")
	if ("AIO100" == current_hostname or "UC100" == current_hostname) and current_hostname ~= hostname then
	...
	uci:set("easycwmp","device","product_class",uci:get("oem",oem.brand,"product_class") or "uc100")
