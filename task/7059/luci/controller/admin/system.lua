...
function index()
	...
	if luci.version.license and ((luci.version.license.fxs and luci.version.license.fxo) or (luci.version.license.gsm or luci.version.license.lte)) then
...
function get_exist_user_namelist()
	local uci = require "luci.model.uci".cursor()
	local users = uci:get_all("user") or {}
	local exist_user="root&daemon&ftp&network&nobody&admin&homeultera&api"
	for k,v in pairs(users) do
		if v['.type'] == "user" then
...
function security()
	...
	if "save" == luci.http.formvalue("action") then
		...
		local msg = ""
		if ("admin" == user or "homeultera" == user) and luci.sys.user.checkpasswd(user,oldpw) then
