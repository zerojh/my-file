...
function authenticator.apiauth(validator, accs, default)
	local pass = luci.http.formvalue("password")
	...
	elseif "/api/login" == luci.http.getenv("PATH_INFO") then
		...
		luci.http.prepare_content("text/plain")
	 	luci.http.write("Invalid session ! Please relogin !")
	 	return false
	end
end
...
function dispatch(request,passport)
	...
	if (c and c.index) or not track.notemplate then
		...
		tpl.context.viewns = setmetatable({
		   ...
		   resource_cbi_js = luci.config.main.resourcebase.."/cbi-1.21.min.js";
	...
	if passport then
		...
		if track.sysauth then
			...
			local loginip = http.getenv("REMOTE_ADDR")
			...
			if sdat then
				...
				setfenv(sdat, {})
				sdat = sdat()
				if (verifytoken or ctx.urltoken.stok == sdat.token) then
					user = sdat.user
				end
			else
