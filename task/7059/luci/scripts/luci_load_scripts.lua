...
function sync_user_to_system(is_restore)
	...
	for k,v in ipairs(passwd_file_user_tb) do
		if "root" ~= v and "daemon" ~= v and "ftp" ~= v and "network" ~= v and "nobody" ~= v and "admin" ~= v and "homeultera" ~= v and "api" ~= v and "" ~= v then
			current_sys_user_list[v]={}
		end
	end

	local web_access_user="\"admin\",\"homeultera\""

	for k,v in pairs(user_list) do
		if "user" == v[".type"] and v.name and "root" ~= v.name and "daemon" ~= v.name and "ftp" ~= v.name and "network" ~= v.name and "nobody" ~= v.name and "admin" ~= v.name and "homeultera" ~= v.name and "api" ~= v.name then
