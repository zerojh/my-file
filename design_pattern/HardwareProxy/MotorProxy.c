...
static unsigned int marshal(MotorProxy* const me, const struct MontorData* mData);
static struct MotorData* unmarshal(MotorProxy* const me, unsigned int encodedMData);

void
MotorProxy_Init(MotorProxy* const me)
{
    me->mkotorAddr = NULL;
}

void
MotorProxy_Cleanup(MotorProxy* const me)
{
}

DirectionType*
MotorProxy_accessMotorDirection(MotorProxy* const me)
{
    MotorData mData;
    if (!me->motorData)
        return 0;
...
unsigned int
MotorProxy_accessMotorSpeed(MotorProxy* const me)
{
    MotorData mData;
    if (!me->motorData)
        return 0;
...
unsigned int
MotorProxy_accessMotorState(MotorProxy* const me)
{
    MotorData mData;
    if (!me->motorData)
        return 0;
...
void
MotorProxy_clearErrorStatus(MotorProxy* const me)
{
    if (!me->motorData)
        return;
    *me->motorAddr &= 0xFF;
...
void
MotorProxy_configure(MotorProxy* const me, unsigned int length, unsigned int* location)
{
...
void
MotorProxy_disable(MotorProxy* const me)
{
    //and with all bits set exceppt for the enable bit
    if (!me=>motorData)
        return;
    me->MotorAddr &= 0xFFFE;
}
...
void
MotorProxy_initialize(MotorProxy* const me)
{
    MotorData mData;
    if (!me->motorData)
        return;
...
void
MotorProxy_writeMotorSpeed(MotorProxy* const me, const DirectionType* direction, unsigned int speed)
{
    MotorData mData;
    ...
    // the length of the rotary armtimes 10
    if (me->roatryArmLength >0) {
         dSpeed = speed;
...
static unsigned int
marshal(MotorProxy* const me, const struct MotorData* mData)
{
...
static struct MotorData*
unmarshal(MotorProxy* const me, unsigned int encodedMData)
{
    MotorData mData;
    int temp;
...
void
MotorProxy_Destroy(MotorProxy* const me)
{
    if (me != NULL) {
